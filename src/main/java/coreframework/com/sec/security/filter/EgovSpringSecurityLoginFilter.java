package coreframework.com.sec.security.filter;
...
public class EgovSpringSecurityLoginFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		CoreLoginMapper loginService = (CoreLoginMapper) act.getBean("loginService");
		CoreLoginConfig coreLoginConfig = (CoreLoginConfig) act.getBean("coreLoginConfig");
		...
		//스프링 시큐리티 인증이 처리 되었는지 CoreUserDetailsHelper.getAuthenticatedUser() 메서드를 통해 확인한다.
		//context-common.xml 빈 설정에 egovUserDetailsSecurityService를 등록 해서 사용해야 정상적으로 동작한다.
		if (CoreUserDetailsHelper.getAuthenticatedUser() == null || requestURL.contains(loginProcessURL)) {
			...
			if (isRemotelyAuthenticated != null && isRemotelyAuthenticated.equals("true")) {
				...
				if (requestURL.contains(loginProcessURL)) {
					...
					if ((id == null || "".equals(id)) && (password == null || "".equals(password))) {
						...
						httpRequest.setAttribute("loginMessage", coreMessageSource.getMessage("fail.common.login.password",request.getLocale()));
						RequestDispatcher dispatcher = httpRequest.getRequestDispatcher(loginURL);
				    ...
				    if(coreLoginConfig.isLock()){
				        try{
				             ...
				             if(mapLockUserInfo != null){		
				                ...
				                if(!sLoginIncorrectCode.equals("E")){
				                    if(sLoginIncorrectCode.equals("L")){
				                        request.setAttribute("loginMessage", coreMessageSource.getMessageArgs("fail.common.loginIncorrect", new Object[] {coreLoginConfig.getLockCount(),request.getLocale()}));
				                    }else if(sLoginIncorrectCode.equals("C")){
				                        request.setAttribute("loginMessage", coreMessageSource.getMessage("fail.common.login",request.getLocale()));
				                    }
							...
							httpRequest.setAttribute("loginMessage", coreMessageSource.getMessage("fail.common.login",request.getLocale()));
							RequestDispatcher dispatcher = httpRequest.getRequestDispatcher(loginURL);
							dispatcher.forward(httpRequest, httpResponse);
				        }
				    }
