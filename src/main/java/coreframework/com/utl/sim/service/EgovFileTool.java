 ...
 */
package coreframework.com.utl.sim.service;
...
import coreframework.com.cmm.util.EgovResourceCloseHelper;
...
public class EgovFileTool {
	...
	public static boolean getExistDirectory(String targetDirPath) throws Exception {
		...
		File f = new File(CoreWebUtil.filePathBlackList(targetDirPath));
	...
	public static List<String> getExistDirectory(String baseDirPath, String targetDirPath, int cnt) throws Exception {
		...
		f = new File(CoreWebUtil.filePathBlackList(baseDirPath));
		isExist = f.exists();
		...
		for (int i = 0; dirList != null && i < dirList.length; i++) {
			...
			f = new File(CoreWebUtil.filePathBlackList(subDirPath));
			...
			if (targetDirPath.equals(dirList[i])) {
				...
				if (new File(CoreWebUtil.filePathBlackList(baseDirPath) + "/" + dirList[i]).isDirectory()) {
					dirFindCnt++;
	...
	public static boolean getExistDirectory(String targetDirPath, String fromDate, String toDate) throws Exception {
		...
		f = new File(CoreWebUtil.filePathBlackList(targetDirPath));
	...
	public static String getLastModifiedDateFromFile(String filePath) {
		...
		File f = null;
		String result = "";
		f = new File(CoreWebUtil.filePathBlackList(filePath));
	...
	public static List<String> getLastDirectoryForModifiedDate(String baseDirPath, String fromDate, String toDate) {
		...
		if (subDirList != null) {
			for (int i = 0; i < subDirList.length; i++) {
				...
				childFile = new File(CoreWebUtil.filePathBlackList(subDirPath));
	...
	public static String deletePath(String filePath) {
		File file = new File(CoreWebUtil.filePathBlackList(filePath));
		String result = "";
	...
	public static String createDirectory(String dirPath) {
		File file = new File(CoreWebUtil.filePathBlackList(dirPath));
		String result = "";
		try {
			if (!file.exists()) {
	...
	public static boolean checkFileExstByName(String dir, String file) throws Exception {
		...
		File srcDrctry = new File(CoreWebUtil.filePathBlackList(drctry));
	...
	public static List<String> getSubFilesByOwner(File[] fileArray, String owner) throws Exception {
		...
		for (int i = 0; i < fileArray.length; i++) {
			...
			if (fileArray[i].isDirectory()) {
				...
				// 파일 생성자 조회
				String fullpath = CoreWebUtil.filePathBlackList(fileArray[i].getAbsolutePath());
				Process p = null;
	...
	public static String deleteDirectory(String dirDeletePath) {
		...
		String result = "";
		File file = new File(CoreWebUtil.filePathBlackList(dirDeletePath));
		if (file.isDirectory()) {
			...
			if (fileList != null) {
				for (int i = 0; i < fileList.length; i++) {
					...
					File f = new File(CoreWebUtil.filePathBlackList(dirDeletePath) + "/" + fileList[i]);
					if (f.isFile()) {
	...
	public static String deleteFile(String fileDeletePath) {
		...
		String result = "";
		File file = new File(CoreWebUtil.filePathBlackList(fileDeletePath));
		if (file.isFile()) {
	...
	public static boolean checkReadAuth(String file) throws Exception {
		...
		File srcFile = new File(CoreWebUtil.filePathBlackList(file1));
	...
	public static List<String> getFileListByDate(String drctry, String updtDate) throws Exception {
		...
		File file = new File(CoreWebUtil.filePathBlackList(drctry1));
	...
	public static Vector<List<String>> parsFileByChar(String parFile, String parChar, int parField) throws Exception {
		...
		File file = new File(CoreWebUtil.filePathBlackList(parFile1));
	...
	public static boolean cmprFilesBySize(String cmprFile1, String cmprFile2) throws Exception {
		...
		File file1 = new File(CoreWebUtil.filePathBlackList(cmprFile11));
		File file2 = new File(CoreWebUtil.filePathBlackList(cmprFile22));
	...
	public static boolean copyFile(String source, String target) throws Exception {
		...
		File srcFile = new File(CoreWebUtil.filePathBlackList(src));
		...
		if (srcFile.exists()) {
			...
			//log.debug("tar:"+tar);
			File tarFile = new File(CoreWebUtil.filePathBlackList(tar));
	...
	public static boolean copyFiles(String[] source, String target) throws Exception {
		...
		for (int i = 0; i < source.length; i++) {
			...
			File chkFile = new File(CoreWebUtil.filePathBlackList(src));
			if (!chkFile.exists()) {
		...
		for (int j = 0; j < source.length; j++) {
			...
			if (result) { //result != false
				...
				// 타겟파일이름 명명
				File chkFile = new File(CoreWebUtil.filePathBlackList(source[j]));
				String tarTemp = tar + FILE_SEPARATOR + chkFile.getName();

				// 복사될 target 파일 생성
				tarTemp = createNewFile(tarTemp);
				File tarFile = new File(CoreWebUtil.filePathBlackList(tarTemp));
			...
			if (fileArray != null) {
				...
				for (int i = 0; i < list.size(); i++) {
					if (result) { //f(result != false){
						...
						// 타겟파일이름 명명
						File chkFile = new File(CoreWebUtil.filePathBlackList(abspath));
						String tarTemp = tar + FILE_SEPARATOR + chkFile.getName();
	...
	public static boolean execCopyFile(File srcFile, File tarFile) throws Exception {
		...
		try {
			...
			if (tarFile1.isDirectory()) {
				tarFile1 = new File(
					CoreWebUtil.filePathBlackList(tarFile1.getAbsolutePath()) + "/" + srcFile.getName());
			}
	...
	public static String deleteDirectory(String dirDeletePath, String dirOwner) {
		...
		try {
			File file = new File(CoreWebUtil.filePathBlackList(dirDeletePath));//KISA 보안약점 조치 (2018-10-29, 윤창원)
	...
	public static String getAccess(String file) throws Exception {
		...
		src = CoreWebUtil.filePathBlackList(src);
	...
	public static boolean copyDirectory(String originalDirPath, String targetDirPath) throws Exception {
		...
		f = new File(CoreWebUtil.filePathBlackList(originalDirPath));
		// 원본이 유효해야 진행한다.
		if (f.exists() && f.isDirectory()) {
			...
			if (targetDirPath1.equals("")) {
				...
				File targetDir = new File(CoreWebUtil.filePathBlackList(targetDirPath1));
				//2017.02.08 	이정은 	시큐어코딩(ES)-부적절한 예외 처리[CWE-253, CWE-440, CWE-754]
				if (targetDir.mkdirs()) {
				...
				if (originalFileList != null) {
					if (originalFileList.length > 0) {
						for (int i = 0; i < originalFileList.length; i++) {
							File subF = new File(
								CoreWebUtil.filePathBlackList(originalDirPath) + FILE_SEPARATOR + originalFileList[i]);
							if (subF.isFile()) {
	...
	public static boolean moveFile(String originalDirPath, String targetDirPath) throws Exception {
		...
		try {
			...
			if (f.exists()) {
				// 타겟으로 설정한 경로가 유효한지 확인(중간경로에 파일명 이 포함되어있으면 유효하지 못하므로 진행안함.
				File targetDir = new File(CoreWebUtil.filePathBlackList(targetDirPath));
				if (targetDir.exists()) {
					// 타겟경로가 이미 있는 경우는 종료
					result = false;
				} else {
	...
	public static boolean startDirectoryMonitering(String targetDirPath) throws Exception {
		...
		try {
			...
			File targetF = new File(CoreWebUtil.filePathBlackList(targetDirPath));
			File logF = new File(CoreWebUtil.filePathBlackList(Globals.CONF_PATH) + "/" + targetF.getName() + ".log");
			...
			if (targetF.exists() && targetF.isDirectory()) {
				...
				if (logF.exists()) {
					...
					if (lastStr.equals("END")) {
						EgovFileMntrg t = new EgovFileMntrg(CoreWebUtil.filePathBlackList(targetDirPath), logF);
						t.start();
					}
				} else {
