package coreframework.com.utl.wed.web;
...
import coreframework.com.utl.fcc.service.EgovFormBasedFileUtil;
import coreframework.com.utl.fcc.service.EgovFormBasedFileVo;
...
public class EgovWebEditorImageController {
     ...
     * @throws Exception
     */
    @RequestMapping(value="/utl/wed/insertImage.cm", method=RequestMethod.GET)
    public String goInsertImage(Model model) throws Exception {

    	return "coreframework/com/utl/wed/EgovInsertImage";
    }
     ...
     * @throws Exception
     */
    @RequestMapping(value="/utl/wed/insertImage.cm", method=RequestMethod.POST)
     ...
     * @throws Exception
     */
    @RequestMapping(value="/utl/wed/insertImageCk.cm", method=RequestMethod.POST)
    public String imageUploadCk(@RequestParam(value="CKEditorFuncNum", required=false) String ckEditorFuncNum, MultipartHttpServletRequest mRequest, HttpServletResponse response, Model model) throws Exception {
		...
		return "coreframework/com/utl/wed/EgovUploadImageComplete";
    }
	...
	private void uploadImageFiles(MultipartHttpServletRequest mRequest, Model model) throws Exception {
		...
		try {
			...
			if (list.size() > 0) {
			    ...
			    model.addAttribute("msg", coreMessageSource.getMessage("success.file.transfer"));
			}
		} catch (SecurityException e) {
			model.addAttribute("url", "");
			model.addAttribute("msg", coreMessageSource.getMessage("errors.file.extension"));
		} catch (Exception e) {
			LOGGER.error(e.getMessage());
			model.addAttribute("url", "");
			model.addAttribute("msg", coreMessageSource.getMessage("errors.file.transfer"));
		}
	}
     ...
     * @throws Exception
     */
    @RequestMapping(value="/utl/web/imageSrc.cm",method=RequestMethod.GET)
