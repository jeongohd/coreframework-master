...
import coreframework.com.cmm.context.CoreWebServletContextListener;
import coreframework.com.cmm.filter.HTMLTagFilter;
import coreframework.com.cmm.filter.SessionTimeoutCookieFilter;
import coreframework.com.cmm.service.EgovProperties;
import coreframework.com.sec.security.filter.EgovSpringSecurityLoginFilter;
import coreframework.com.sec.security.filter.EgovSpringSecurityLogoutFilter;
import coreframework.com.uat.uap.filter.CoreLoginPolicyFilter;
import coreframework.com.utl.wed.filter.CkFilter;
...
/**
 * CoreWebApplicationInitializer 클래스
 * <Notice>
 ...
 *   2018.10.26  신용호           CoreLoginPolicyFilter 추가 (IP접근처리)
...
public class CoreWebApplicationInitializer implements WebApplicationInitializer {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(CoreWebApplicationInitializer.class);
	@Override
	public void onStartup(ServletContext servletContext) throws ServletException {
		LOGGER.debug("CoreWebApplicationInitializer START-============================================");
			
		//-------------------------------------------------------------
		// Egov Web ServletContextListener 설정
		//-------------------------------------------------------------
		servletContext.addListener(new CoreWebServletContextListener());
		...
		characterEncoding.addMappingForUrlPatterns(null, false, "*.cm");
		//characterEncoding.addMappingForUrlPatterns(EnumSet.of(DispatcherType.REQUEST), true, "*.cm");
		
		//-------------------------------------------------------------
		// Spring ServletContextListener 설정
		//-------------------------------------------------------------
		XmlWebApplicationContext rootContext = new XmlWebApplicationContext();
		rootContext.setConfigLocations(new String[] { "classpath*:coreframework/spring/com/**/context-*.xml" });
		//rootContext.setConfigLocations(new String[] { "classpath*:coreframework/spring/com/context-*.xml","classpath*:coreframework/spring/com/*/context-*.xml" });
		rootContext.refresh();
		rootContext.start();
		...
		xmlWebApplicationContext.setConfigLocation("/WEB-INF/config/coreframework/springmvc/core-com-*.xml");
		...
		if("security".equals(EgovProperties.getProperty("Globals.Auth").trim())) {
			...
			//로그인 실패시 반활 될 URL설정
			egovSpringSecurityLoginFilter.setInitParameter("loginURL", "/uat/uia/coreLoginUsr.cm");
			//로그인 처리 URL설정
			egovSpringSecurityLoginFilter.setInitParameter("loginProcessURL", "/uat/uia/actionLogin.cm");
			//처리 Url Pattern
			egovSpringSecurityLoginFilter.addMappingForUrlPatterns(null, false, "*.cm");
			...
			egovSpringSecurityLogoutFilter.addMappingForUrlPatterns(null, false, "/uat/uia/actionLogout.cm");
		
		} else if("session".equals(EgovProperties.getProperty("Globals.Auth").trim())) {
			//-------------------------------------------------------------
			// CoreLoginPolicyFilter 설정
			//-------------------------------------------------------------	
			FilterRegistration.Dynamic egovLoginPolicyFilter = servletContext.addFilter("LoginPolicyFilter", new CoreLoginPolicyFilter());
			egovLoginPolicyFilter.addMappingForUrlPatterns(null, false, "/uat/uia/actionLogin.cm");
		...
		regCkFilter.setInitParameter("properties", "coreframework/coreProps/ck.properties");
		...
		multipartFilter.addMappingForUrlPatterns(null, false, "*.cm");
		...
		htmlTagFilter.addMappingForUrlPatterns(null, false, "*.cm");
		...
		sessionTimeoutFilter.addMappingForUrlPatterns(null, false, "*.cm");
		...
		LOGGER.debug("CoreWebApplicationInitializer END-============================================");
