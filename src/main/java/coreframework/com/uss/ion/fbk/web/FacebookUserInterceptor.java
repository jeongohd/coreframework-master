...
import coreframework.com.uss.ion.fbk.service.FacebookSecurityContext;
import coreframework.com.uss.ion.fbk.service.FacebookUser;
...
public final class FacebookUserInterceptor extends HandlerInterceptorAdapter {
	...
	private void handleSignOut(HttpServletRequest request, HttpServletResponse response) {
		if (FacebookSecurityContext.userSignedIn() && request.getServletPath().startsWith("/uss/ion/fbk/facebookSignout.cm")) {
	...
	private boolean requestForSignIn(HttpServletRequest request) {
		return request.getServletPath().startsWith("/uss/ion/fbk/facebookSignin.cm");
	}

	private boolean requireSignIn(HttpServletRequest request, HttpServletResponse response) throws Exception {
		new RedirectView("/uss/ion/fbk/facebookSignin.cm", true).render(null, request, response);
		return false;
	}
