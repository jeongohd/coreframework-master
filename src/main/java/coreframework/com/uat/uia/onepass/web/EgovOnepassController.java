package coreframework.com.uat.uia.onepass.web;
...
import coreframework.com.uat.uia.onepass.EgovOnepassService;
...
public class EgovOnepassController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/uat/uia/onepass/onepassLogin.cm", method=RequestMethod.POST)
	public String onepassLogin(HttpServletRequest request, HttpServletResponse response, ModelMap model) {
		...
		try {
			if ("LOGIN".equals(serviceType)) {
				...
				action = request.getContextPath()+"/index.cm";
				inputName = "";
				inputValue = "";
			}
		} catch (Exception e) {
			return "coreframework/com/cmm/error/onepassAccessDenied";
		}
		...
		return "coreframework/com/uat/uia/onepass/onepassLogin";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/uat/uia/onepass/onepassCallback.cm", method=RequestMethod.POST)
	public String onepassCallback(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		if (onepassResponse.getStatus() == STATUS.SUCCESS  && onepassResponse.getResultCode() == RESULT_CODE.SUCCESS) {
			...
			if (onepassResponse.getType() == TYPE.LOGIN) {
			    ...
			    if (findOnepassUser != null && findOnepassUser.getStatus() == USER_STATUS.USE) {
					...
					if (usedCnt > 0) {
						...
						model.addAttribute("resultMessage", coreMessageSource.getMessageArgs("digital.onepass.connect.success", new Object[]{resultVO.getId()}));
						model.addAttribute("redirectUrl", request.getContextPath()+"/index.cm");
						return "coreframework/com/uat/uia/onepass/onepassResult";

					}

					// 기관 아이디 조회 실패
					else {
						model.addAttribute("resultMessage", coreMessageSource.getMessage("digital.onepass.connect.join.failure"));
						model.addAttribute("redirectUrl", request.getContextPath()+"/index.cm");
						return "coreframework/com/uat/uia/onepass/onepassResult";
					}
			    ...
			    // 사용자 정보 조회 실패
			    	...
			    	model.addAttribute("resultMessage", coreMessageSource.getMessage("digital.onepass.connect.user.infomation.failure"));
			    	model.addAttribute("redirectUrl", request.getContextPath()+"/index.cm");
			    	return "coreframework/com/uat/uia/onepass/onepassResult";
			    }
			...
			// type 이 LOGIN 이 아님
		    	...
		    	model.addAttribute("resultMessage", coreMessageSource.getMessage("digital.onepass.connect.athentication.failure"));
		    	model.addAttribute("redirectUrl", request.getContextPath()+"/index.cm");
		    	return "coreframework/com/uat/uia/onepass/onepassResult";
		    }
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/uat/uia/onepass/onepassCancel.cm", method=RequestMethod.POST)
	public String onepassCancel(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
		...
		if (Objects.isNull(onepassUser)) {
			...
			model.addAttribute("resultMessage", coreMessageSource.getMessage("digital.onepass.connect.athentication.failure"));
			model.addAttribute("redirectUrl", request.getContextPath()+"/uat/uia/actionLogout.cm");
			return "coreframework/com/uat/uia/onepass/onepassResult";
		} else {
			if (onepassUser.getProcess_result() == PROCESS_RESULT.SUCESS) {
				model.addAttribute("resultMessage", coreMessageSource.getMessage("digital.onepass.disconnect.success"));
				model.addAttribute("redirectUrl", request.getContextPath()+"/uat/uia/actionLogout.cm");
				return "coreframework/com/uat/uia/onepass/onepassResult";
			} else {
				model.addAttribute("resultMessage", coreMessageSource.getMessage("digital.onepass.connect.user.infomation.failure"));
