package coreframework.com.cop.ems.web;
...
import coreframework.com.cop.ems.service.EgovSndngMailDetailService;
...
public class EgovSndngMailDetailController {
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/cop/ems/selectSndngMailDetail.cm")
	public String selectSndngMail(@ModelAttribute("sndngMailVO") SndngMailVO sndngMailVO, ModelMap model) throws Exception {
		...
		if (!resultMailVO.getMssageId().equals("")) {
			// 발송메일 상세조회 화면 이동
			return "coreframework/com/cop/ems/EgovMailDetail";
		} else {
			// 오류 페이지 이동
			return "coreframework/com/cmm/coreError";
		}
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/cop/ems/deleteSndngMail.cm")
	public String deleteSndngMail(@ModelAttribute("sndngMailVO") SndngMailVO sndngMailVO, ModelMap model) throws Exception {
		...
		// 3. 발송메일 목록 페이지 이동
		return "redirect:/cop/ems/selectSndngMailList.cm";
	}
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/cop/ems/backSndngMailDetail.cm")
	 ...
	 * @exception Exception
	 */
	@RequestMapping(value = "/cop/ems/selectSndngMailXml.cm")
	public void selectSndngMailXml(@ModelAttribute("sndngMailVO") SndngMailVO sndngMailVO, HttpServletResponse response, ModelMap model) throws Exception {
		...
		File uFile = new File(CoreWebUtil.filePathBlackList(xmlFile));
		...
		if (fSize > 0) {
			...
			response.setHeader("Content-Disposition", "attachment; filename=\"" + CoreWebUtil.removeCRLF(uFile.getName()) + "\"");
			...
			printwriter.println("<br><br><br><h2>Could not get file name:<br>" + CoreWebUtil.clearXSSMinimum(xmlFile) + "</h2>");
