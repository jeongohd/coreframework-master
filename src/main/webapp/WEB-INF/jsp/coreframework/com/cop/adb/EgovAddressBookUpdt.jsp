...
<script type="text/javascript" src="<c:url value='/js/coreframework/com/cmm/popup.js'/>" ></script>
...
<script type="text/javascript">
	function fn_egov_update_adbkInf(form){
		...
		if(confirm("<spring:message code="common.save.msg" />")){
			document.adbk.action = "<c:url value='/cop/adb/UpdateAddressBook.cm'/>";
	...
	function fn_egov_deleteUser(userId){
		...
		document.adbk.action = "<c:url value='/cop/adb/deleteUser.cm'/>";
		document.adbk.submit();
	}

	function fn_egov_select_adbkInfs(){
		document.adbk.action = "<c:url value='/cop/adb/selectAdbkList.cm'/>";
		document.adbk.submit();
	}
	function fn_egov_inqire_user(){
		var retVal;
		var url = "<c:url value='/cop/adb/openPopup.cm?requestUrl=/cop/adb/selectManList.cm&width=850&height=360'/>";
		...
		if(retVal != null){
			...
			document.adbk.action = "<c:url value='/cop/adb/addUser.cm'/>";
			document.adbk.submit();
...
<form name="adbk" action="${pageContext.request.contextPath}/cop/adb/UpdateAddressBook.cm"  method="post" onSubmit="fn_egov_update_adbkInf(document.forms[0]); return false;">
...
<div class="wTableFrm">
	...
	<tbody>
		...
		<tr>
			...
			<td class="left">
			   ...
			   <a href="#" onClick="fn_egov_inqire_user();return false;"><img src="<c:url value='/images/coreframework/com/cmm/btn/btn_search.gif'/>" align="middle" style="border:0px" alt=<spring:message code="comCopAdb.regist.findAdbkUser"/> title="<spring:message code="comCopAdb.regist.findAdbkUser"/>"></a>
			</td>
		</tr>
	 ...
	 <tbody>
		 <c:forEach var="result" items="${searchVO.adbkMan}" varStatus="status">
		  <tr>
			...
			<td>
				<!-- 2011.09.05 수정사항 -->
				<!-- <form name="deleteItem" method="post" action="<c:url value='/cop/adb/deleteUser.cm'/>"> -->
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/cop/adb/selectAdbkList.cm' />?pageIndex=<c:out value='${searchVO.pageIndex}'/>"  title="<spring:message code="button.list" />  <spring:message code="input.button" />"><spring:message code="button.list" /></a></span>
