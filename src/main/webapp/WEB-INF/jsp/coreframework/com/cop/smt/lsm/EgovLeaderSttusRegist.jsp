...
<c:set var="ImgUrl" value="/images/coreframework/com/cop/smt/lsm/"/>
<c:set var="CssUrl" value="/css/coreframework/com/cop/smt/lsm/"/>
<!DOCTYPE html>
<html lang="ko">
<head>
...
<script type="text/javascript">
	...
	function fn_egov_insert_leadersttus() {
		...
		if (confirm('<spring:message code="common.regist.msg" />')) {
			document.leaderSttusVO.action = "<c:url value='/cop/smt/lsm/mng/insertLeaderSttus.cm'/>";
	...
	function fn_egov_list_leadersttus(){
		document.leaderSttusVO.action = "<c:url value='/cop/smt/lsm/mng/selectLeaderSttusList.cm'/>";
		document.leaderSttusVO.submit();	
	}	
	...
	******************************************************** */
	function fn_egov_schdulCharger_LeaderSchdule(strTitle, frmUniqId, frmEmplNo, frmEmplyrNm, frmOrgnztNm){
		...
		arrParam[5] = frmOrgnztNm;
	 	...
	 	window.showModalDialog("<c:url value='/cop/smt/lsm/selectEmplyrListPopup.cm' />", arrParam,"dialogWidth=800px;dialogHeight=500px;resizable=yes;center=yes");
	}
...
<form:form modelAttribute="leaderSttusVO" name="leaderSttusVO" method="post" action="${pageContext.request.contextPath}/cop/smt/lsm/mng/insertLeaderSttus.cm' />">

	<table width="100%" cellpadding="8" class="table-search" border="0">
	 <tr>
	  <td width="100%"class="title_left">
	   <h1><img src="<c:url value='/images/coreframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" style="vertical-align: middle" alt="제목아이콘이미지">
	   &nbsp;간부상태 등록</h1></td>
	 </tr>
	</table>
	...
	<tbody>
	  ...
	  <tr> 
		...
		<tr>
			...
			<a href="<c:url value='/cop/smt/lsm/selectEmplyrListPopup.cm' />" target="_blank"  title="새 창으로 이동"  onClick="fn_egov_schdulCharger_LeaderSchdule('간부', 'leaderId', '', 'leaderNm', 'orgnztNm');return false;">
	...
	<tr>
      ...
      <td>&nbsp;&nbsp;</td>
      <td><span class="button"><a href="<c:url value='/cop/smt/lsm/mng/selectLeaderSttusList.cm'/>?searchWrd=<c:out value='${leaderSttusVO.searchWrd}'/>&amp;searchCnd=<c:out value='${leaderSttusVO.searchCnd}'/>&amp;pageIndex=<c:out value='${leaderSttusVO.pageIndex}'/>" onclick="fn_egov_list_leadersttus(); return false;"><spring:message code="button.list" /></a></span></td>
	</tr>
	</table>
	</div>
	<!-- 검색조건 유지 -->
