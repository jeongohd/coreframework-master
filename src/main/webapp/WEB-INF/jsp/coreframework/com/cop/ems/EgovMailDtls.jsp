...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/cop/ems/selectSndngMailList.cm' />";
 ...
 ******************************************************** */
function fnRegist(){
	document.listForm.action = "<c:url value='/cop/ems/insertSndngMailView.cm' />";
   	document.listForm.submit();
}
/* ********************************************************
 * 상세회면 처리 함수
...
function fnDetail(mid){
	document.detailForm.action = "<c:url value='/cop/ems/selectSndngMailDetail.cm' />";
...
function fnDelete(){
    ...
    if(checkedIds.length > 0) {
    	...
    	if (ret == true) {
    		...
    		document.deleteForm.action = "<c:url value='/cop/ems/deleteSndngMailList.cm' />";
...
<div class="board">
		...
		<form name="listForm" action="<c:url value='/cop/ems/selectSndngMailList.cm'/>" method="post">
		...
		<tr>
			...
			<td><a href="<c:url value='/cop/ems/selectSndngMailDetail.cm' />" onclick="fnDetail('${resultInfo.mssageId}'); return false;">${resultInfo.sj}</a></td>
		...
		<div style="display:none;">
			<!-- 상세조회할 발송메일ID를 담는 폼 -->
			<form name="detailForm" action="<c:url value='/cop/ems/selectSndngMailDetail.cm'/>" method="post">
				<input name="mssageId" type="hidden" value=""/>
				<input type="submit" id="invisible" class="invisible"/>
			</form>
			<!-- 삭제할 발송메일ID(여러 ID를 ,로 묶어 만들어진 데이터)를 담는 폼 -->
			<form name="deleteForm" action="<c:url value='/cop/ems/deleteSndngMailList.cm'/>" method="post">
				<input name="mssageId" type="hidden" value=""/>
