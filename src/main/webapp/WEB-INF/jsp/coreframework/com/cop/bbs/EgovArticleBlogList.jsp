...
<link type="text/css" rel="stylesheet" href="<c:url value='/css/coreframework/com/cop/bbs/style.css' />">
<script src="<c:url value='/js/coreframework/com/cmm/jquery-1.12.4.min.js' />"></script>
...
function fn_egov_select_linkPage(pageNo){
	...
	document.articleForm.action = "<c:url value='/cop/bbs/selectArticleBlogList.cm'/>";
...
function fn_egov_loadBdList(bbsId,blogNm,cnt){
	...
	$(".comm_List").empty();
	$.ajax({
		url :"<c:url value='/cop/bbs/selectArticleBlogDetail.cm'/>"
        ,type: "POST"
...
function fn_blog_cn(blogId){
	...
	location.href="<c:url value='/cop/bbs/insertArticleView.cm' />?bbsId="+bbsId+"&blogAt=Y&blogId="+blogId;
}

function fn_clickComm(bbsId, nttId, ntcrId, replyPosblAt, blogId, cnt){
	$.ajax({
		url :"<c:url value='/cop/bbs/selectArticleBlogDetailCn.cm'/>"
        ,type: "POST"
...
function fn_egov_insert_commentList(bbsId, nttId, blogId) {
	...
	document.formComment.modified.value = false;
  	document.formComment.action = "<c:url value='/cop/cmt/insertArticleComment.cm'/>";
	...
	<div class="blog_header">
		...
		<div class="h_menu">
			...
			<c:choose>
				...
				<li><a href="<c:url value='/cop/bbs/insertBBSMasterView.cm?blogId=${boardMasterVO.blogId}&blogAt=Y' />" onclick="fn_egov_ls()"><spring:message code="comCopBlog.articleBlogList.regCategory" /></a></li><!-- 카테고리등록 -->
				<li><a href="<c:url value='/cop/bbs/selectBlogListManager.cm?blogId=${boardMasterVO.blogId}' />" ><spring:message code="comCopBlog.blogUseMgrMain.btnBoard" /></a></li> <!-- 개인블로그관리  -->
			</ul>
