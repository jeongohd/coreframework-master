...
<script type="text/javaScript" language="javascript">
	...
	******************************************************** */
	...
	function fnEgovSchdulSe(setValue) {
		...
		location.href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulDailyList.cm' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&day=<%=iNowDay%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;
	}

	function fnEgovSchdulSelect() {

		var newLocation = "<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulDailyList.cm' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&day=<%=iNowDay%>&searchCondition=LEADER_ID&searchKeyword=" + document.leaderSchdulVO.schdulSe.options[document.leaderSchdulVO.schdulSe.selectedIndex].value + "&searchKeywordEx=" + document.leaderSchdulVO.leaderName.value;
...
<body> 
<form name="leaderSchdulVO" id="leaderSchdulVO" action="<c:url value='/cop/smt/lsm/usr/EgovLeaderSchdulDailyList.cm' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&day=<%=iNowDay%>&searchCondition=SCHDUL_SE" method="post">
<div id="content" style="width:712px;">
	...
	<div class="search_box2 mb10">
		...
		<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulDailyList.cm' />?year=<%=iNowYear-1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>" style="vertical-align:3px"><img alt="이전년도" src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png' />" /></a>
		<span class="t1"><%=iNowYear%><spring:message code="comCopSmtLsm.leaderSchdulMonthList.year" /></span>
		<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulDailyList.cm' />?year=<%=iNowYear+1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>" style="margin-right:16px; vertical-align:3px"><img alt="다음년도" src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png' />" /></a>
		
		<%if(iNowMonth > 0 ){ %>
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth-1%>&amp;day=<%=iNowDay%>" style="vertical-align:3px"><img alt="이전월" src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png' />" /></a>
		<%}%>
		<span class="t1"><%=iNowMonth+1%><spring:message code="comCopSmtLsm.leaderSchdulMonthList.month" /></span>
		<%if(iNowMonth < 11 ){ %>
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth+1%>&amp;day=<%=iNowDay%>" style="margin-right:16px; vertical-align:3px"><img alt="다음월" src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png' />" /></a>
		<%}%>
		
		<%if(iNowDay > 1 ){ %>
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay-1%>" style="vertical-align:3px"><img alt="이전주" src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png' />" />
		</a>
		<%}%>
		<span class="t1"><%=iNowDay%></span>
		<%if(iNowDay < iEndDay ){ %>
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay+1%>" style="margin-right:16px; vertical-align:3px"><img alt="다음주" src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png' />" /></a>
		<%}%>
	</div>
...
%>
		<tr>  
		    <td>
		       	<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdul.cm' />?schdulId=<%=(String)leaderSchdulVO.getSchdulId()%>&schdulDe=<%=sUseDate %>&searchMode=DAILY&year=<%=iNowYear %>&month=<%=iNowMonth %>&day=<%=iNowDay %>" target="parent" onClick="JavaScript:fn_egov_detail_LeaderSchdul('<%=leaderSchdulVO.getSchdulId() %>','<%=sUseDate %>')"/>
<%
...
<%	
	...
	if(!sSchdulBgnDate.equals(sSchdulEndDate)){
		//out.print("<table><tr><td nowrap><div class='divDotText' style='width:350px;border:solid 0px;'><a href=\"<c:url value='/cop/smt/lsm/usr/selectLeaderSchdul.cm' />?schdulId=" + (String)leaderSchdulVO.getSchdulId() + "&amp;schdulDe=" + sUseDate + "&amp;searchMode=DAILY&amp;year=" + iNowYear + "&amp;month=" + iNowMonth + "&amp;day=" + iNowDay + "\" target=\"_parent\" onClick=\"JavaScript:fn_egov_detail_LeaderSchdul('" + leaderSchdulVO.getSchdulId() + "','" + sUseDate + "')\">");
%>
				<table>
					<tr>
						<td nowrap>
...
%>
    			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdul.cm' />?schdulId=<%=(String)leaderSchdulVO.getSchdulId()%>&schdulDe=<%=sUseDate %>&searchMode=DAILY&year=<%=iNowYear %>&month=<%=iNowMonth %>&day=<%=iNowDay %>" target="parent" onClick="JavaScript:fn_egov_detail_LeaderSchdul('<%=leaderSchdulVO.getSchdulId() %>','<%=sUseDate %>')"/>
<%
		//out.print("<a href=\"<c:url value='/cop/smt/lsm/usr/selectLeaderSchdul.cm' />?schdulId=" + (String)leaderSchdulVO.getSchdulId() + "&amp;schdulDe=" + sUseDate + "&amp;searchMode=DAILY&amp;year=" + iNowYear + "&amp;month=" + iNowMonth + "&amp;day=" + iNowDay + "\" target=\"_parent\" onClick=\"JavaScript:fn_egov_detail_LeaderSchdul('" + leaderSchdulVO.getSchdulId() + "','" + sUseDate + "')\">");
