...
function fn_egov_select_linkPage(pageNo){
	...
	document.BlogMasterForm.action = "<c:url value='/cop/bbs/selectBlogMasterList.cm'/>";
...
function fn_insert_Blog() {
	document.BlogMasterForm.action = "<c:url value='/cop/bbs/insertBlogMasterView.cm'/>";
  	document.BlogMasterForm.submit();
}

function fn_userChk(userId) {
	$.ajax({
		url :"<c:url value='/cop/bbs/selectChkBloguser.cm'/>"
        ,type: "POST"
        ,dataType: 'json'  	   
...
<form name="BlogMasterForm" action="<c:url value='/cop/bbs/selectBlogList.cm'/>" method="post" onSubmit="fn_egov_search_blog(); return false;">
<div class="board">
	...
	<tr>
		...
		<td class="left"><a href="<c:url value='/cop/bbs/selectArticleBlogList.cm'/>?blogId=${resultInfo.blogId}&bbsId=${resultInfo.bbsId}"><c:out value='${fn:substring(resultInfo.blogNm, 0, 40)}'/></a></td>
	...
	<!-- 
	<div class="btn">
		<span class="btn_s"><a href="<c:url value='/cop/blog/insertBlogMasterView.cm' />"  title="<spring:message code="button.create" /> <spring:message code="input.button" />"><spring:message code="button.create" /></a></span>
	</div>
	-->
