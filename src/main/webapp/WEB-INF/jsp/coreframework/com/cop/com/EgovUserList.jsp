...
<script type="text/javascript">
	...
	function fn_egov_select_userInfo(pageIndex) {
		...
		if (_target == 'selectClubOprtrList') {
			actionUrl = "<c:url value='/cop/com/selectClubOprtrList.cm'/>";
		} else if (_target == 'selectClubUserList') {
			actionUrl = "<c:url value='/cop/com/selectClubUserList.cm'/>";
		} else if (_target == 'selectCmmntyMngrList') {
			actionUrl = "<c:url value='/cop/com/selectCmmntyMngrList.cm'/>";
		} else if (_target == 'selectCmmntyUserList') {
			actionUrl = "<c:url value='/cop/com/selectCmmntyUserList.cm'/>";
		} else {
			actionUrl = "<c:url value='/cop/com/selectUserList.cm'/>";
		}
	...
	function fn_egov_deleteUser(emplyrId) {
		...
		if (confirm('<spring:message code="cop.withdraw.msg" />')) {
			...
			if (_target == 'selectCmmntyUserList') {
				url = "<c:url value='/cop/com/deleteCmmntyUser.cm'/>";
				document.frm.param_cmmntyId.value = document.frm.trgetId.value;
			} else if (_target == 'selectClubUserList') {
				url = "<c:url value='/cop/com/deleteClbUser.cm'/>";
	...
	function fn_egov_reRegistUser(emplyrId){
		...
		if (confirm('<spring:message code="cop.reregist.msg" />')) {
			if (_target == 'selectCmmntyUserList') {
				url = "<c:url value='/cop/com/reRegistCmmntyUser.cm'/>";
				document.frm.param_cmmntyId.value = document.frm.trgetId.value;
			} else if (_target == 'selectClubUserList') {
				url = "<c:url value='/cop/com/reRegistClbUser.cm'/>";
				document.frm.param_clbId.value = document.frm.trgetId.value;
			}
	...
	function fn_egov_registManager(emplyrId) {
		...
		if (confirm('<spring:message code="cop.registmanager.msg" />')) {
			if (_target == 'selectCmmntyUserList') {
				url = "<c:url value='/cop/com/registCmmntyManager.cm'/>";
				document.frm.param_cmmntyId.value = document.frm.trgetId.value;
			} else if (_target == 'selectClubUserList') {
				url = "<c:url value='/cop/com/registClbOprtr.cm'/>";
				document.frm.param_clbId.value = document.frm.trgetId.value;
			}
			document.frm.param_emplyrId.value = emplyrId;
			document.frm.action = url;
	...
	<table width="100%" cellpadding="8" class="table-search" border="0">
	 <tr>
	  <td width="40%"class="title_left">
	   <img src="<c:url value='/images/coreframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" align="middle" alt="제목버튼이미지">
	   &nbsp;사용자 목록</td>
	  <th >
	  </th>
	  <td width="10%" >
	  ...
	  <th width="10%">
	   <table border="0" cellspacing="0" cellpadding="0">
	    <tr>
	    <!-- 
	      <td><img src="<c:url value='/images/coreframework/com/cmm/btn/bu2_left.gif' />" width="8" height="20" alt="button left"></td>
