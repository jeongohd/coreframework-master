...
String sImgUrl = "/images/coreframework/com/cmm/";
String sCssUrl = "/css/coreframework/com/cmm/";
...
<HEAD>
	...
	<link type="text/css" rel="stylesheet" href="<c:url value='/css/coreframework/com/cop/smt/sdm/dept_schdule_manage.css'/>">
	...
	******************************************************** */
	function fn_egov_regist_DeptSchdulManage(sDate){
		...
		gOpener.action = "<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageRegist.cm' />";
		gOpener.submit();
	}
	...
	******************************************************** */
	function fn_egov_detail_DeptSchdulManage(schdulId){
		...
		gOpener.action = "<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDetail.cm' />";
		gOpener.submit();
	}
	...
	function fnEgovSchdulSe(setValue) {
		...
		location.href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDailyList.cm' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&day=<%=iNowDay%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;
	}
	
	function fnEgovSchdulTodate() {

		location.href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.cm' />?year=<%=sTodate.substring(0, 4)%>&month=<%=Integer.valueOf(sTodate.substring(4, 6))-1%>&day=<%=sTodate.substring(6, 8)%>&searchCondition=SCHDUL_SE&searchKeyword=" + document.deptSchdulManageVO.schdulSe.value;
...
<div class="sort_area">
	...
	<div class="date_view">
		<ul>
			...
			<li><a class="prev" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDailyList.cm' />?year=<%=iNowYear-1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>">prev</a></li>
			<li class="date"><%=iNowYear%> <spring:message code="comCopSmtSdm.Navi.Year"/> </li>  <!-- 년 -->
			<li><a class="next" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDailyList.cm' />?year=<%=iNowYear+1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>">next</a></li>
			
			<li class="date">&nbsp;</li>
			
			<%if(iNowMonth > 0 ){ %>
			<li><a class="prev" href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth-1%>&amp;day=<%=iNowDay%>">prev</a></li>
			<%}%>
			<li class="date"><%=iNowMonth+1%> <spring:message code="comCopSmtSdm.Navi.Month"/> </li> <!-- 월 -->
			<%if(iNowMonth < 11 ){ %>
			<li><a class="next" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth+1%>&amp;day=<%=iNowDay%>">next</a></li>
			<%}%>
			<li class="date">&nbsp;</li>
			<%if(iNowDay > 1 ){ %>
			<li><a class="prev" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay-1%>">prev</a></li>
			<%}%>
			<li class="date"><%=iNowDay%> <spring:message code="comCopSmtSdm.Navi.Day"/> </li> <!-- 일 -->
			<%if(iNowDay < iEndDay ){ %>
			<li><a class="next" href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay+1%>">next</a></li>
			<%}%>		
		</ul>
	</div>
</div>
...
%>
  <tr>
    ...
    //out.print("<a href=\"<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDetail.cm' />?schdulId=" + (String)egovMap.get("schdulId") + "\" target=\"_top\">");
    %>
	<a href="<c:url value='/cop/smt/sim/EgovIndvdlSchdulManageDetail.cm' />?schdulId=<%=(String)egovMap.get("schdulId")%>" target="_parent">
	<%
