...
<%@ page import="coreframework.com.cop.smt.lsm.service.LeaderSchdulVO"%>
...
<%@page import="coreframework.com.utl.fcc.service.EgovDateUtil"%><html lang="ko">
<head>
...
<script type="text/javaScript" language="javascript">
	...
	******************************************************** */
	function fn_egov_regist_LeaderSchdul(sDate){
		...
		gOpener.action = '<c:url value="/cop/smt/lsm/mng/addLeaderSchdul.cm"/>';
		gOpener.submit();
	}
	...
	******************************************************** */
	function fn_egov_detail_LeaderSchdul(schdulId, schdulDe){
		...
		gOpener.week.value = "<%=iNowWeek%>";
		gOpener.searchMode.value = "WEEK";
		gOpener.action = '<c:url value="/cop/smt/lsm/usr/selectLeaderSchdul.cm"/>';
		gOpener.submit();
	}
	...
	function fnEgovSchdulSe(setValue) {
		...
		location.href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&week=<%=iNowWeek%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;
	}

	function fnEgovSchdulSelect() {

		var newLocation = "<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&week=<%=iNowWeek%>&searchCondition=LEADER_ID&searchKeyword=" + document.leaderSchdulVO.schdulSe.options[document.leaderSchdulVO.schdulSe.selectedIndex].value + "&searchKeywordEx=" + document.leaderSchdulVO.leaderName.value;
...
<body style="margin:0"> 
<form name="leaderSchdulVO" id="leaderSchdulVO" action="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&week=<%=iNowWeek%>&searchCondition=SCHDUL_SE" method="post">
<div id="content" style="width:712px;">
	...
	<div class="search_box2 mb10">
		...
		<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear-1%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek%>" style="vertical-align:3px"><img alt="이전년도" src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png' />" /></a>
		<span class="t1"><%=iNowYear%><spring:message code="comCopSmtLsm.leaderSchdulMonthList.year" /></span><!-- 년 -->
		<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear+1%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek%>" style="margin-right:16px; vertical-align:3px"><img alt="다음년도" src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png' />" /></a>
		
		<%if(iNowMonth > 0 ){ %>
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth-1%>&amp;week=<%=iNowWeek%>" style="vertical-align:3px"><img alt="이전월" src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png' />" /></a>
		<%}%>
		<span class="t1"><%=iNowMonth+1%><spring:message code="comCopSmtLsm.leaderSchdulMonthList.month" /></span><!-- 월 -->
		<%if(iNowMonth < 11 ){ %>
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth+1%>&amp;week=<%=iNowWeek%>" style="margin-right:16px; vertical-align:3px"><img alt="다음월" src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png' />" /></a>
		<%}%>
		
		<%if(iNowWeek > 0 ){ %>
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek-1%>" style="vertical-align:3px"><img alt="이전주" src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png' />" />
		</a>
		<%}%>
		...
		<%if(iNowWeek < listWeekGrop.size()-1 ){ %>
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulWeekList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek+1%>" style="margin-right:16px; vertical-align:3px"><img alt="다음주" src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png' />" /></a>
		<%}%>
	</div>
...
%>
		<tr>
			<td>
			    <a href="<c:url value='/cop/smt/lsm/mng/addLeaderSchdul.cm' />?schdulBgnDe=<%=iUseDate%>&amp;schdulEndDe=<%=iUseDate%>&amp;searchMode=WEEK&amp;year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;week=<%=iNowWeek%>" target="_parent" onClick="JavaScript:fn_egov_regist_LeaderSchdul('<%=iUseDate%>');">
	...
	if(listResult != null){ 
		...
		for(int j=0;j < listResult.size(); j++){
			...
			if(iUseDate == iSchdulDate){
				out.print("<table><tr><td nowrap><div class='divDotText' style='width:120px;border:solid 0px;'><a href=\"" + request.getContextPath() + "/cop/smt/lsm/usr/selectLeaderSchdul.cm?schdulId=" + (String)leaderSchdulVO.getSchdulId() +  "&amp;schdulDe=" + iUseDate + "&amp;searchMode=WEEK&amp;year=" + iNowYear + "&amp;month=" + iNowMonth + "&amp;week=" + iNowWeek + "\" target=\"_parent\" onClick=\"JavaScript:fn_egov_detail_LeaderSchdul('" + (String)leaderSchdulVO.getSchdulId() + "', '" + iUseDate + "')\">");
    ...
    if(listResult != null){ 
		...
		for(int j=0;j < listResult.size(); j++){
			...
			if(iUseDate == iSchdulDate && !sSchdulBgnDate.equals(sSchdulEndDate)){
				out.print("<table><tr><td nowrap><div class='divDotText' style='width:350px;border:solid 0px;'><a href=\"" + request.getContextPath() + "/cop/smt/lsm/usr/selectLeaderSchdul.cm?schdulId=" + (String)leaderSchdulVO.getSchdulId() +  "&amp;schdulDe=" + iUseDate + "&amp;searchMode=WEEK&amp;year=" + iNowYear + "&amp;month=" + iNowMonth + "&amp;week=" + iNowWeek + "\" target=\"_parent\" onClick=\"JavaScript:fn_egov_detail_LeaderSchdul('" + (String)leaderSchdulVO.getSchdulId() + "', '" + iUseDate + "')\">");
				out.print("[");
...
<%
    if(listResult != null){ 
		...
		for(int j=0;j < listResult.size(); j++){
			...
			if(iUseDate == iSchdulDate){
				out.print("<table><tr><td nowrap><div class='divDotText' style='width:60px;border:solid 0px;'><a href=\"" + request.getContextPath() + "/cop/smt/lsm/usr/selectLeaderSchdul.cm?schdulId=" + (String)leaderSchdulVO.getSchdulId() +  "&amp;schdulDe=" + iUseDate + "&amp;searchMode=WEEK&amp;year=" + iNowYear + "&amp;month=" + iNowMonth + "&amp;week=" + iNowWeek + "\" target=\"_parent\" onClick=\"JavaScript:fn_egov_detail_LeaderSchdul('" + (String)leaderSchdulVO.getSchdulId() + "', '" + iUseDate + "')\">");
