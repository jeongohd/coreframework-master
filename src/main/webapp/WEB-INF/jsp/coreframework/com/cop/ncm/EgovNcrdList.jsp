...
<script type="text/javascript">
	...
	function fn_egov_select_ncrdInfs(pageNo) {
		...
		document.frm.action = "<c:url value='/cop/ncm/selectNcrdInfs.cm'/>";
		document.frm.submit();
	}
	...
	function fn_egov_popupNcrdInf(ncrdId){
		window.open("<c:url value='/cop/ncm/selectNcrdInfPopup.cm' />?ncrdId="+ncrdId,"<spring:message code="comCopNcm.ncrdList.nrcdSearch" />","width=640, height=350");/* 명함조회 */
		
	}

	function fn_egov_registNcrd(ncrdId){
		document.frm.ncrdId.value = ncrdId;
		document.frm.action = "<c:url value='/cop/ncm/insertNcrdUseInf.cm'/>";
		document.frm.submit();
	}
</script>
...
<body>
<form name="frm" method="post" action="<c:url value='/cop/ncm/selectNcrdInfs.cm'/>">
