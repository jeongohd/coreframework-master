...
<script type="text/javascript">
	...
	function fn_egov_modify_memoreprt() {
		document.memoReprtVO.action = "<c:url value='/cop/smt/mrm/modifyMemoReprt.cm'/>";
	...
	function fn_egov_delete_memoreprt(){
		if(confirm("<spring:message code="common.delete.msg"/>")){/* 삭제 하시겠습니까? */
			document.memoReprtVO.action = "<c:url value='/cop/smt/mrm/deleteMemoReprt.cm'/>";
			document.memoReprtVO.submit();
		}
	}
	...
	function fn_egov_confirm_memoreprt(){
		if(confirm("<spring:message code="comCopSmtMrm.memoReprtDetail.memoreprt.confirm"/>")){/* 의견등록을 하시겠습니까? */
			document.memoReprtVO.action = "<c:url value='/cop/smt/mrm/updateMemoReprtDrctMatter.cm'/>";
			document.memoReprtVO.submit();
		}
	}
...
<form:form modelAttribute="memoReprtVO" name="memoReprtVO" method="post" action="${pageContext.request.contextPath}/cop/smt/mrm/updateMemoReprt.cm' />">
...
<div class="wTableFrm">
	...
	<div class="btn">
		<c:if test="${memoReprt.reportrId eq uniqId}">
			<span class="btn_s" style="width:64px !important"><a href="<c:url value='/cop/smt/mrm/updateMemoReprtDrctMatter.cm'/>?searchWrd=<c:out value='${memoReprtVO.searchWrd}'/>&amp;searchCnd=<c:out value='${memoReprtVO.searchCnd}'/>&amp;pageIndex=<c:out value='${memoReprtVO.pageIndex}'/>&amp;searchSttus=<c:out value='${memoReprtVO.searchSttus}'/>&amp;searchDrctMatter=<c:out value='${memoReprtVO.searchDrctMatter}'/>&amp;searchBgnDe=<c:out value='${memoReprtVO.searchBgnDe}'/>&amp;searchEndDe=<c:out value='${memoReprtVO.searchEndDe}'/>" onclick="fn_egov_confirm_memoreprt(); return false;"><spring:message code="comCopSmtMrm.memoReprtDetail.memoreprt"/></a></span><!-- 의견등록 -->
		</c:if>
		<c:if test="${memoReprt.reprtSttus eq ('미확인' or  'Unidentified') && memoReprt.wrterId eq uniqId}">
			<input class="s_submit" type="submit" value='<spring:message code="button.update" />' onclick="fn_egov_modify_memoreprt(); return false;" />
			<span class="btn_s"><a href="<c:url value='/cop/smt/mrm/deleteMemoReprt.cm'/>?reprtId=<c:out value='${memoReprtVO.reprtId}'/>" onclick="fn_egov_delete_memoreprt(); return false;"><spring:message code="button.delete" /></a></span>
		</c:if>
