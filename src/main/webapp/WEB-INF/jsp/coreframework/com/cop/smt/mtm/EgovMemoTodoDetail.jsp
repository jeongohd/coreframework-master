...
<script type="text/javascript">
	...
	function fn_egov_update_memotodo() {
		document.memoTodoVO.action = "<c:url value='/cop/smt/mtm/modifyMemoTodo.cm'/>";
		document.memoTodoVO.submit();					
	}

	function fn_egov_delete_memotodo(){
		if(confirm("<spring:message code="common.delete.msg"/>")){/* 삭제하시겠습니까? */
			document.memoTodoVO.action = "<c:url value='/cop/smt/mtm/deleteMemoTodo.cm'/>";
			document.memoTodoVO.submit();
		}
	}
...
<form:form modelAttribute="memoTodoVO" name="memoTodoVO" method="post" action="${pageContext.request.contextPath}/cop/smt/mtm/modifyMemoTodo.cm' />">
...
<div class="wTableFrm">
	...
	<div class="btn">		
		...
		<span class="btn_s"><a href="<c:url value='/cop/smt/mtm/deleteMemoTodo.cm'/>?todoId=<c:out value='${memoTodoVO.todoId}'/>" onclick="fn_egov_delete_memotodo(); return false;"><spring:message code="button.delete" /></a></span>
      <% 
	  ...
	  %>
		<span class="btn_s"><a href="<c:url value='/cop/smt/mtm/selectMemoTodoListToday.cm'/>" onclick=""><spring:message code="button.list" /></a></span>
	  <%
	  	}
      %>
	</div>
