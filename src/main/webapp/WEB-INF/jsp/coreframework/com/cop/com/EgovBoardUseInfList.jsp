...
<script type="text/javascript">
	...
	function fn_egov_select_bbsUseInfs(pageNo){
		...
		document.frm.action = "<c:url value='/cop/com/selectBBSUseInfs.cm'/>";
		document.frm.submit();
	}
	function fn_egov_insert_addbbsUseInf(){
		document.frm.action = "<c:url value='/cop/com/addBBSUseInf.cm'/>";
		document.frm.submit();
	}
	function fn_egov_select_bbsUseInf(bbsId, trgetId){
		document.frm.bbsId.value = bbsId;
		document.frm.trgetId.value = trgetId;
		document.frm.action = "<c:url value='/cop/com/selectBBSUseInf.cm'/>";
		document.frm.submit();
	}
...
<form name="frm" method="post" action = "<c:url value='/cop/com/selectBBSUseInf.cm'/>">
	...
	<table width="100%" cellpadding="8" class="table-search" border="0">
	 <tr>
	  <td width="40%"class="title_left">
	  	<h1>
	  		<img src="<c:url value='/images/coreframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" align="middle" alt="제목아이콘이미지">&nbsp;게시판 사용정보 조회
	  	</h1>
	  </td>
	  <th >
	  </th>
	  ...
	  <th width="10%">
	   <table border="0" cellspacing="0" cellpadding="0">
	    <tr>
	      ...
	      <a href="<c:url value='/cop/com/selectBBSUseInfs.cm'/>" onclick="fn_egov_select_bbsUseInfs('1'); return false;">조회</a>
	      </span></td>
	      <td>&nbsp;&nbsp;</td>
	      <td><span class="button">
	      <a href="<c:url value='/cop/com/addBBSUseInf.cm'/>" onclick="fn_egov_insert_addbbsUseInf(); return false;">등록</a>
	      </span></td>
	    </tr>
	   </table>
	  </th>
	    ...
	    <c:choose>
	    	<c:when test="${useCommunity == 'true'}">
	    		<td class="lt_text" nowrap>
			    	...
			    	<!-- 2010.11.1
			    	<form name="item" method="post" action="<c:url value='/cop/com/selectBBSUseInf.cm'/>"  target="_blank">
					...
					<a href="<c:url value='/cop/com/selectBBSUseInf.cm'/>?bbsId=<c:out value='${result.bbsId}'/>&amp;trgetId=<c:out value='${result.trgetId}'/>" onclick="">
