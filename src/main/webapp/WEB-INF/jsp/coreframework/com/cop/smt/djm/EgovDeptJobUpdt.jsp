...
<script type="text/javascript">
	function fn_egov_update_deptjob() {
		...
		if (confirm('<spring:message code="common.regist.msg" />')) {
			document.deptJobVO.action = "<c:url value='/cop/smt/djm/updateDeptJob.cm'/>";
	...
	function fn_egov_list_deptjob(){
		document.deptJobVO.action = "<c:url value='/cop/smt/djm/selectDeptJobList.cm'/>";
		document.deptJobVO.submit();	
	}	
	...
	******************************************************** */
	function fn_egov_dept_DeptJobBx(strType){
		...
		window.showModalDialog("<c:url value='/cop/smt/djm/selectDeptJobBxListPopup.cm' />", arrParam,"dialogWidth=800px;dialogHeight=500px;resizable=yes;center=yes");
	}
	...
	******************************************************** */
	function fn_egov_charger_DeptJob(strTitle, frmUniqId, frmEmplNo, frmEmplyrNm, frmOrgnztNm){
		...
		arrParam[5] = frmOrgnztNm;
	 	...
	 	window.showModalDialog("<c:url value='/cop/smt/djm/selectChargerListPopup.cm' />", arrParam,"dialogWidth=800px;dialogHeight=500px;resizable=yes;center=yes");
	}
</script>
</head>
<body>
<noscript class="noScriptTitle"><spring:message code="common.noScriptTitle.msg" /></noscript>
<form:form modelAttribute="deptJobVO" name="deptJobVO" method="post" action="${pageContext.request.contextPath}/cop/smt/djm/updateDeptJob.cm' />" enctype="multipart/form-data">
<div class="wTableFrm">
	<!-- 타이틀 -->
	...
	<table class="wTable">
		...
		<tr>
			...
			<td class="left">
				...
				<a href="<c:url value='/cop/smt/djm/selectDeptJobBxListPopup.cm' />" target="_blank" title="새 창으로 이동" onclick="fn_egov_dept_DeptJobBx('typeDeptJobBx');return false;">
					<img alt="부서업무함" src="<c:url value='/images/coreframework/com/cmm/btn/btn_search.gif' />" title="부서업무함" />
				</a>
		...
		<tr>
			...
			<td class="left">
				...
				<a href="<c:url value='/cop/smt/djm/selectChargerListPopup.cm' />" target="_blank" title="새 창으로 이동" onclick="fn_egov_charger_DeptJob('담당자', 'chargerId', '', 'chargerNm', '');return false;">
					<img alt="담당자" src="<c:url value='/images/coreframework/com/cmm/btn/btn_search.gif' />" title="담당자">
				</a>
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/cop/smt/djm/selectDeptJobList.cm'/>?searchWrd=<c:out value='${deptJobVO.searchWrd}'/>&amp;searchCnd=<c:out value='${deptJobVO.searchCnd}'/>&amp;pageIndex=<c:out value='${deptJobVO.pageIndex}'/>&amp;searchDeptId=<c:out value='${deptJobVO.searchDeptId}'/>&amp;searchDeptJobBxId=<c:out value='${deptJobVO.searchDeptJobBxId}'/>" onclick="fn_egov_list_deptjob(); return false;"><spring:message code="button.list" /></a></span>
	</div>
	
	<input type="hidden" name="returnUrl" value="<c:url value='/cop/smt/djm/modifyDeptJob.cm' />" />
