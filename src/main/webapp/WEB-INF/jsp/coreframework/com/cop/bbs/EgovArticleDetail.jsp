 ...
 ******************************************************** */
 function fn_egov_reply_article() {
		document.articleForm.action = "<c:url value='/cop/bbs/replyArticleView.cm'/>";
...
function fn_egov_updt_commentList() {
	...
	if (confirm('<spring:message code="common.update.msg" />')) {
		...
		form.action = "<c:url value='/cop/cmt/updateArticleComment.cm'/>";
		form.submit();
	}
}
...
function fn_egov_selectCommentForupdt(commentNo) {
	...
	form.action = "<c:url value='/cop/bbs/selectArticleDetail.cm'/>";
	form.submit();
}
...
function fn_egov_deleteCommentList(commentNo) {
	...
	if (confirm('<spring:message code="common.delete.msg" />')) {
		form.modified.value = "true";
		form.commentNo.value = commentNo;
		form.action = "<c:url value='/cop/cmt/deleteArticleComment.cm'/>";
		form.submit();
	}
}
...
<c:import url="/cop/stf/selectSatisfactionList.cm" charEncoding="utf-8">
...
<div class="wTableFrm">
	...
	<tbody>
		...
		<tr>
			...
			<td colspan="5">
				<c:import url="/cmm/fms/selectFileInfs.cm" charEncoding="utf-8">
	...
	<div class="btn">
		...
		<!-- 익명글 수정/삭제 불가  -->
		<form name="articleForm" action="<c:url value='/cop/bbs/updateArticleView.cm'/>" method="post" style="float:left;">
		...
		<form name="formDelete" action="<c:url value='/cop/bbs/deleteArticle.cm'/>" method="post" style="float:left; margin:0 0 0 3px;">
		...
		<form name="formReply" action="<c:url value='/cop/bbs/replyArticleView.cm'/>" method="post" style="float:left; margin:0 0 0 3px;">
			<input type="submit" class="s_submit" value="<spring:message code="button.reply" />"><!-- 답글 -->
			<input name="nttId" type="hidden" value="<c:out value="${result.nttId}" />">
			<input name="bbsId" type="hidden" value="<c:out value="${boardMasterVO.bbsId}" />">
		</form>
		</c:if>
		<form name="formList" action="<c:url value='/cop/bbs/selectArticleList.cm'/>" method="post" style="float:left; margin:0 0 0 3px;">
			<input type="submit" class="s_submit" value="<spring:message code="button.list" />"><!-- 목록 -->
			<input name="bbsId" type="hidden" value="<c:out value="${boardMasterVO.bbsId}" />">
		</form>
		<form name="formScrap" action="<c:url value='/cop/scp/insertArticleScrapView.cm'/>" method="post" style="float:left; margin:0 0 0 3px;">
...
<c:if test="${useComment == 'true'}">
	<c:import url="/cop/cmt/selectArticleCommentList.cm" charEncoding="utf-8">
	</c:import>
</c:if>
...
<c:if test="${useSatisfaction == 'true'}">
	...
	<input name="nttId" type="hidden" value="<c:out value="${result.nttId}" />">
	<input name="bbsId" type="hidden" value="<c:out value="${boardMasterVO.bbsId}" />">
	<c:import url="/cop/stf${prefix}/selectSatisfactionList.cm" charEncoding="utf-8">
