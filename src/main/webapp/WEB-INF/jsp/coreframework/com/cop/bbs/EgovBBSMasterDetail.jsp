 ...
 ******************************************************** */
 function fn_egov_delete_bbs(bbsId){
	if(confirm("<spring:message code="common.delete.msg" />")){	
		...
		document.BBSMasterForm.action = "<c:url value='/cop/bbs/deleteBBSMaster.cm'/>";
...
<form name="BBSMasterForm" action="<c:url value='/cop/bbs/updateBBSMasterView.cm'/>" method="post">
<div class="wTableFrm">
	<!-- 타이틀 -->
	...
	<tbody>
		...
		<c:if test="${result.useAt == 'Y' }">
			<tr>
				...
				<td colspan="5" class="cnt">
				<a href="<c:url value='/cop/bbs/selectArticleList.cm?bbsId=${result.bbsId}' />">/cop/bbs/selectArticleList.cm?bbsId=${result.bbsId }</a>
				</td>
			</tr>
		</c:if>
		<tr>
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/cop/bbs/selectBBSMasterInfs.cm' /><c:if test='${result.cmmntyId != null}'>?cmmntyId=${result.cmmntyId}</c:if>"  title="<spring:message code="title.list" /> <spring:message code="input.button" />"><spring:message code="button.list" /></a></span><!-- 목록 -->
