...
<script type="text/javascript">
	...
	function fn_egov_insert_sms() {
		document.frm.action = "<c:url value='/cop/sms/addSms.cm'/>";
		document.frm.submit();
	}

	function fn_egov_select_sms(pageNo) {
		document.frm.pageIndex.value = pageNo;
		document.frm.action = "<c:url value='/cop/sms/selectSmsList.cm'/>";
		document.frm.submit();
	}

	function fn_egov_inqire_sms(smsId) {
		document.frm.smsId.value = smsId;
		document.frm.action = "<c:url value='/cop/sms/selectSms.cm'/>";
		document.frm.submit();
	}
</script>
...
<div class="board">
	...
	<form name="frm" method="post" action="<c:url value='/cop/sms/selectSmsList.cm'/>">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="s_btn"><a href="<c:url value='/cop/sms/addSms.cm'/>?pageIndex=<c:out value='${searchVO.pageIndex}'/>" onclick="fn_egov_insert_sms(); return false;"><spring:message code="cop.sms.send"/></a></span><!-- 전송 -->
			</li>
		</ul>
	</div>
	</form>
	...
	<table class="board_list">
		...
		<tbody>
			...
			<tr>
				...
				<td>	
				<a href="<c:url value='/cop/sms/selectSms.cm'/>?smsId=${result.smsId}&pageIndex=${searchVO.pageIndex}" onClick="fn_egov_inqire_sms('<c:out value="${result.smsId}"/>');return false;"><c:out value='${result.frstRegisterPnttm}'/></a>
