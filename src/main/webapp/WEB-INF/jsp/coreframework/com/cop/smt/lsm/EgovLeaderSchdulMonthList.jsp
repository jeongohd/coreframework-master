...
<%@ page import='coreframework.com.utl.fcc.service.EgovNumberUtil' %>
<%@ page import='coreframework.com.utl.fcc.service.EgovStringUtil' %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="spring" uri="http://www.springframework.org/tags"%>

<%
java.util.Calendar cal = java.util.Calendar.getInstance();
String sImgUrl = "/images/coreframework/com/cop/smt/lsm/";
String sCssUrl = "/css/coreframework/com/cop/smt/lsm/";
...
<script type="text/javaScript" language="javascript">
	...
	******************************************************** */
	...
	function fnEgovSchdulSe(setValue) {
		...
		location.href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulMonthList.cm' />?year=<%=year%>&month=<%=month%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;
	...
	function fnEgovSchdulSelect() {
		...
		var newLocation = "<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulMonthList.cm' />?year=<%=year%>&month=<%=month%>&searchCondition=LEADER_ID&searchKeyword=";
...
<body style="margin:0"> 
<form name="leaderSchdulVO" id="leaderSchdulVO" action="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulMonthList.cm' />?year=<%=year%>&month=<%=month%>&searchCondition=LEADER_ID" method="post" target="_self">
	<div id="content" style="width:712px;">
		...
		<div class="search_box2 mb10">
			...
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulMonthList.cm'/>?year=<%=year-1%>&amp;month=<%=month%>" style="vertical-align:-4px"><img alt="이전년도" src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png'/>" /></a>
			<span class="t1"><%=year%><spring:message code="comCopSmtLsm.leaderSchdulMonthList.year" /></span><!-- 년 -->
			<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulMonthList.cm'/>?year=<%=year+1%>&amp;month=<%=month%>" style="margin-right:16px; vertical-align:-4px"><img alt="다음년도" src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png'/>" /></a>
			
			
			
			<%if(month > 0 ){ %>
				<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulMonthList.cm'/>?year=<%=year%>&amp;month=<%=month-1%>" style="vertical-align:-4px"><img alt="이전월" src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png'/>" /></a>
			<%}%>
			<span class="t1"><%=month+1%><spring:message code="comCopSmtLsm.leaderSchdulMonthList.month" /></span><!-- 월 -->
			<%if(month < 11 ){ %>
				<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdulMonthList.cm'/>?year=<%=year%>&amp;month=<%=month+1%>" style="vertical-align:-4px"><img alt="다음월" src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png'/>" /></a>
			<%}%>
		</div>
...
for(int index = 1; index <= endDay; index++)
{
	...
	out.println("<td>");
%>	
	<a href="<c:url value='/cop/smt/lsm/mng/addLeaderSchdul.cm' />?schdulBgnDe=<%=iUseDate%>&amp;schdulEndDe=<%=iUseDate%>&amp;searchMode=MONTH&amp;year=<%=year%>&amp;month=<%=month%>" target="_parent" onclick="JavaScript:fn_egov_regist_LeaderSchdul('<%=iUseDate%>');"><font color="<%=color%>"><%=index%></font></a>
<%	
	out.println("<br />");
					...
					<tr>
						<td>
							<div class='divDotText'>
								<a href="<c:url value='/cop/smt/lsm/usr/selectLeaderSchdul.cm' />?schdulId=<%=schdulId%>&amp;schdulDe=<%=iUseDate%>&amp;searchMode=MONTH&amp;year=<%=year%>&amp;month=<%=month%>" target="_parent" onClick="JavaScript:fn_egov_detail_LeaderSchdul('<%=schdulId %>', '<%=iUseDate%>')">
