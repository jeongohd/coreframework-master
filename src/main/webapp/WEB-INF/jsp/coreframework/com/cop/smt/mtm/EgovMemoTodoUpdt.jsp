...
<title><spring:message code="comCopSmtMtm.memoTodoUpdt.title"/></title><!-- 메모할일 수정 -->
...
function initCalendar(){
	...
	function fn_egov_update_memotodo() {
		...
		if (confirm('<spring:message code="common.regist.msg" />')) {
			document.memoTodoVO.action = "<c:url value='/cop/smt/mtm/updateMemoTodo.cm'/>";
	...
	function fn_egov_list_memotodo(){
		document.memoTodoVO.action = "<c:url value='/cop/smt/mtm/selectMemoTodoList.cm'/>";
		document.memoTodoVO.submit();
	}
...
<form:form modelAttribute="memoTodoVO" name="memoTodoVO" method="post" action="${pageContext.request.contextPath}/cop/smt/mtm/updateMemoTodo.cm' />">

<div class="wTableFrm">
	<!-- 타이틀 -->
	<h2><spring:message code="comCopSmtMtm.memoTodoUpdt.title"/></h2><!-- 메모할일 수정 -->
	...
	<table class="wTable">
		...
		<tr>
			...
			<td class="left">
			    <form:input path="todoDe" maxlength="10" readonly="true" title="할일일자" cssStyle="width:70px"/>
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/cop/smt/mtm/selectMemoTodoList.cm'/>?searchWrd=<c:out value='${memoTodoVO.searchWrd}'/>&amp;searchCnd=<c:out value='${memoTodoVO.searchCnd}'/>&amp;pageIndex=<c:out value='${memoTodoVO.pageIndex}'/>&amp;searchDe=<c:out value='${memoTodoVO.searchDe}'/>&amp;searchBgnDe=<c:out value='${memoTodoVO.searchBgnDe}'/>&amp;searchEndDe=<c:out value='${memoTodoVO.searchEndDe}'/>" onclick="fn_egov_list_memotodo(); return false;"><spring:message code="button.list" /></a></span>
	</div>
