...
<script type="text/javascript">
	...
	function fn_egov_select_ncrdInfs(pageNo){
		...
		document.frm.action = "<c:url value='/cop/ncm/selectMyNcrdUseInf.cm'/>";
		document.frm.submit();
	}
	function fn_egov_addNcrdInf(){
		document.frm.action = "<c:url value='/cop/ncm/addNcrdInf.cm'/>";
		document.frm.submit();
	}
	function fn_egov_update_ncrdInf(ncrdId){
		document.frm.ncrdId.value = ncrdId;
		document.frm.action = "<c:url value='/cop/ncm/selectNcrdInf.cm'/>";
		document.frm.submit();
	}
	function fn_egov_popupNcrdInf(ncrdId){
		window.open("<c:url value='/cop/ncm/selectNcrdInfPopup.cm' />?ncrdId="+ncrdId,"<spring:message code="comCopNcm.ncrdList.nrcdSearch" />","width=640, height=350");
	}
</script>
</head>
<body>
<form name="frm" method="post" action="<c:url value='/cop/ncm/selectMyNcrdUseInf.cm'/>">
				...
				<td>
					<c:choose>
			    		<c:when test="${result.frstRegisterId == uniqId}">
							<a href="<c:url value='/cop/ncm/selectNcrdInf.cm?ncrdId='/><c:out value='${result.ncrdId}'/>">
							<c:out value="${result.ncrdNm}"/></a>
			    		</c:when>
			    		<c:otherwise>
							<a href="<c:url value='/cop/ncm/selectNcrdInfPopup.cm?ncrdId='/><c:out value='${result.ncrdId}'/>" >
			    			<c:out value="${result.ncrdNm}"/></a>
			    		</c:otherwise>
			    	</c:choose>
				</td>
