...
String sImgUrl = "/images/coreframework/com/cop/smt/sdm/";
String sCssUrl = "/css/coreframework/com/cop/smt/sdm/";
...
<HEAD>
	...
	******************************************************** */
	function fn_egov_regist_DeptSchdulManage(sDate){
		...
		gOpener.action = "<c:url value='/cop/smt/sdm/EgovDeptSchdulManageRegist.cm' />";
		gOpener.submit();
	}
	...
	******************************************************** */
	function fn_egov_detail_DeptSchdulManage(schdulId){
		...
		gOpener.action ="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDetail.cm' />";
		gOpener.submit();
	}
	...
	function fnEgovSchdulSe(setValue) {
		...
		location.href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.cm' />?year=<%=iNowYear%>&month=<%=iNowMonth%>&day=<%=iNowDay%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;
	}
...
<div class="sort_area">
	...
	<div class="date_view">
		<ul>
			...
			<li><a class="prev" href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.cm' />?year=<%=iNowYear-1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>">prev</a></li>
			<li class="date"><%=iNowYear%> <spring:message code="comCopSmtSdm.Navi.Year"/> </li>  <!-- 년 -->
			<li><a class="next" href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.cm' />?year=<%=iNowYear+1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>">next</a></li>
			...
			<%if(iNowMonth < 11 ){ %>
			<li><a class="next" href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth+1%>&amp;day=<%=iNowDay%>">next</a></li>
			<%}%>
			<li class="date">&nbsp;</li>
			<%if(iNowDay > 1 ){ %>
			<li><a class="prev" href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay-1%>">prev</a></li>
			<%}%>
			<li class="date"><%=iNowDay%> <spring:message code="comCopSmtSdm.Navi.Day"/> </li> <!-- 일 -->
			<%if(iNowDay < iEndDay ){ %>
			<li><a class="next" href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.cm' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay+1%>">next</a></li>
			<%}%>
