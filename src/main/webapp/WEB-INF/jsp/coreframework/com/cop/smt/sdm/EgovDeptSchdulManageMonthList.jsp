...
<HEAD>
	...
	******************************************************** */
	...
	function fnEgovSchdulSe(setValue) {
		...
		location.href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.cm' />?year=<%=year%>&month=<%=month%>&searchCondition=SCHDUL_SE&searchKeyword=" + setValue;

	}

	function fnEgovSchdulTodate() {

		location.href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.cm' />?year=<%=sTodate.substring(0, 4)%>&month=<%=Integer.valueOf(sTodate.substring(4, 6))-1%>&searchCondition=SCHDUL_SE&searchKeyword=" + document.deptSchdulManageVO.schdulSe.value;
...
<div class="sort_area">
	...
	<div class="date_view" style="left:39%;">
		<ul>
			...
			<li><a class="prev" href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.cm' />?year=<%=year-1%>&amp;month=<%=month%>">prev</a></li>
			<li class="date"><%=year%> <spring:message code="comCopSmtSdm.Navi.Year"/></li><!-- 년 -->
			<li><a class="next" href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.cm' />?year=<%=year+1%>&amp;month=<%=month%>">next</a></li>
			<li class="date">&nbsp;</li>
			
			<%if(month > 0 ){ %>
			<li><a href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.cm' />?year=<%=year%>&month=<%=month-1%>" title="<spring:message code="comCopSmtSdm.goPrevMonth"/>" class="prev">prev</a></li><!-- 이전달로 가기 -->
			<%}%>
			<li class="date"><%=month+1%> <spring:message code="comCopSmtSdm.Navi.Month"/></li><!-- 월 -->
			<%if(month < 11 ){ %>
			<li><a href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageMonthList.cm' />?year=<%=year%>&month=<%=month+1%>" class="next" title="<spring:message code="comCopSmtSdm.goNextMonth"/>">next</a></li><!-- 다음달로 가기 -->
			<%}%>
