...
<c:set var="ImgUrl" value="/images/coreframework/com/cop/com/"/>
<%
 /**
...
<script type="text/javascript">
	...
	function fn_egov_inqire_confirmInfo(cnfmNo,trgetId) {
		...
		document.frm.action = "<c:url value='/cop/com/forUpdateConfirmRequest.cm'/>";
		document.frm.submit();
	}
</script>
<title>승인 목록</title>
...
<form name="frm" action ="<c:url value='/cop/com/forUpdateConfirmRequest.cm' />" method="post">
	...
	<table width="100%" cellpadding="8" class="table-search" border="0">
	 <tr>
	  <td width="40%"class="title_left">
	   <img src="<c:url value='/images/coreframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" align="middle" alt="제목아이콘이미지">
	   &nbsp;승인 목록</td>
	  <th >
	  </th>
	  <td width="10%" >
		    ...
		    <td class="lt_text3" nowrap>
		    	<c:choose>
		    		<c:when test="${result.confmSttusCode == 'AP01'}">
						<input type="hidden" name="confmNumber" value="0" />
						<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">
						<!-- a href="<c:url value='/cop/cmy/selectCmmntyInf.cm'/>?confmNumber=0"; onclick="">
							<c:out value="${result.confmTyCodeNm}"/>
						</a-->
						<!-- a href="<c:url value='/cop/com/forUpdateConfirmRequest.cm'/>?confmNumber=0"; onclick="">
							<c:out value="${result.confmTyCodeNm}"/>
						</a-->
						<!-- 2011.9.7 수정분 -->
