...
<script type="text/javascript">
	function fn_egov_regist_tmplatInfo(){
		...
		if (confirm('<spring:message code="common.regist.msg" />')) {
			document.templateInf.action = "<c:url value='/cop/tpl/insertTemplateInf.cm'/>";
			document.templateInf.submit();
		}
	}

	function fn_egov_select_tmplatInfo(){
		document.templateInf.action = "<c:url value='/cop/tpl/selectTemplateInfs.cm'/>";
		document.templateInf.submit();
	}
	...
	function fn_egov_previewTmplat() {
		...
		if (frm.tmplatSeCode.value == 'TMPT01') {
			target = "<c:url value='/cop/bbs/previewBoardList.cm' />";
			width = "750";
		} else if (frm.tmplatSeCode.value == 'TMPT02') {
			target = "<c:url value='/cop/cmy/previewCmmntyMainPage.cm' />";
			width = "980";
		} else if (frm.tmplatSeCode.value == 'TMPT03') {
			target = "<c:url value='/cop/cus/previewClubMainPage.cm' />";
			width = "980";
		} else {
