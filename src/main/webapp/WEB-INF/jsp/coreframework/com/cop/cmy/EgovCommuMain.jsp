...
<!--
	...
	function fn_egov_loadBdList(bbsId, bbsAttrbCode, bbsTyCode){
		var url = "<c:url value='/cop/bbs/selectArticleList.cm' />?bbsId="+bbsId+"&bbsAttrbCode="+bbsAttrbCode;
		document.getElementById("contentFrame").src = url;
	}

	function fn_egov_registUser(cmmntyId){
		document.frm.action = "<c:url value='/cop/cmy/insertCommuUserBySelf.cm' />";
		document.frm.submit();
	}

	function fn_egov_deleteUser(cmmntyId){
		document.frm.action = "<c:url value='/cop/cmy/deleteCommuUserBySelf.cm' />";
		document.frm.submit();
	}
...
<div id="cmnt_wrap">
	...
	<div id="cmnt_header">
		...
		<div class="h_menu">
			<ul class="gnb">
                ...
                <li><a href="" onclick="fn_egov_loadMngrMenu('<c:url value="/cop/bbs/selectBBSMasterInfs.cm"/>'); return false;"><spring:message code="comCopCmy.communityUseMgrMain.btnBoard" /></a></li> <!-- 게시판관리  -->
                <li><a href="" onclick="fn_egov_loadMngrMenu('<c:url value="/cop/cmy/selectCommuUserList.cm"/>'); return false;"><spring:message code="comCopCmy.communityUseMgrMain.btnUser" /></a></li> <!-- 사용자관리  -->
                </c:if>
                <!-- 관리자 메뉴 부분 End -->
			</ul>
		</div>
	...
	<div id="cmnt_container">
		...
		<!-- contents 영역 -->
		<div id="cmnt_content">
		<iframe id="contentFrame" src="<c:url value='/cop/cmy/cmmntyMainContents.cm'/>?cmmntyId=<c:out value='${cmmntyVO.cmmntyId}' />" width="768" height="680" title="<spring:message code="comCopCmy.communityUseMgrMain.titleContents" />"></iframe><!-- 컨텐츠영역 -->
		</div>
		<!-- contents 영역 //-->
	</div>
	<!-- container //-->
