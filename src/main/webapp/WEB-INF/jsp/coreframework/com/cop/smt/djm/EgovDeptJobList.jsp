...
<script type="text/javascript">
	...
	function fn_egov_select_deptjob(pageNo) {
		...
		document.frm.action = "<c:url value='/cop/smt/djm/selectDeptJobList.cm'/>";
		document.frm.submit();	
	}
	...
	******************************************************** */
	function fn_egov_inqire_deptjob(deptJobId) {
		...
		document.frm.action = "<c:url value='/cop/smt/djm/selectDeptJob.cm'/>";
		document.frm.submit();	
	}
	...
	******************************************************** */
	function fn_egov_insert_deptjob(){	
		document.frm.action = "<c:url value='/cop/smt/djm/addDeptJob.cm'/>";
		document.frm.submit();
	}
...
<div class="board">
	...
	<form name="frm" method="post" action="<c:url value='/cop/smt/djm/selectDeptJobList.cm'/>">
					...
					<c:forEach var="resultBx" items="${resultBxList}" varStatus="st">
						...
						<c:set var="tmpDeptNm" value="${resultBx.deptNm}"/>
							...
							<table width="100%" cellpadding="1" class="table-list" summary="<spring:message code="comCopSmtDjm.deptJobList.summary" />"><!-- 부서업무함에 대한 목록을 제공합니다. -->
								...
								<tr>
									<td>&nbsp;&nbsp;
										<form name="searchVO" method="post" action="<c:url value='/cop/smt/djm/selectDeptJobList.cm'/>">
					...
					<tbody>
						<c:forEach var="result" items="${resultList}" varStatus="status">
							...
							<tr>
								...
								<td>
									<form name="deptJobVO" method="post" action="<c:url value='/cop/smt/djm/selectDeptJob.cm'/>">
