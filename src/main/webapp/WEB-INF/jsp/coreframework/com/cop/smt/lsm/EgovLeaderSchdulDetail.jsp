...
%>
<c:set var="sLinkType" value="<%=CoreWebUtil.clearXSSMaximum(sLinkType)%>"/>
<!DOCTYPE html>
<html lang="ko">
<head>
...
function fn_egov_modify_LeaderSchdul(){
	var vFrom = document.leaderSchdulVO;
	vFrom.action = "<c:url value='/cop/smt/lsm/mng/modifyLeaderSchdul.cm' />";;
	vFrom.submit();
...
function fn_egov_delete_LeaderSchdul(){
	...
	if(confirm("<spring:message code="common.delete.msg" />")){/* 삭제하시겠습니까? */
		vFrom.action = "<c:url value='/cop/smt/lsm/mng/deleteLeaderSchdul.cm' />";
		vFrom.submit();
	}
}
</script>
...
<div class="wTableFrm">
	...
	<form name="leaderSchdulVO" id="leaderSchdulVO" action="<c:url value='/cop/smt/lsm/mng/modifyLeaderSchdul.cm'/>" method="post">
	<!-- 등록폼 -->
	<table class="wTable">
		<colgroup>
			<col style="width:16%" />
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/cop/smt/lsm/mng/deleteLeaderSchdul.cm'/>?schdulId=<c:out value='${leaderSchdulVO.schdulId}'/>" onclick="fn_egov_delete_LeaderSchdul(); return false;"><spring:message code="button.delete" /></a></span>
