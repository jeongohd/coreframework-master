...
<script type="text/javascript">
	...
	function fn_egov_downFile(atchFileId, fileSn){
		window.open("<c:url value='/cmm/fms/FileDown.cm?atchFileId="+atchFileId+"&fileSn="+fileSn+"'/>");
	}
	...
	function fn_egov_deleteFile(atchFileId, fileSn, delTrName) {
		...
		newForm.method = "post";
		newForm.action = "<c:url value='/cmm/fms/deleteFileInfs.cm'/>";
		...
		<tr id="egov_file_view_table_tr_${status.count}">
			...
			<c:choose>
				<c:when test="${updateFlag eq 'Y'}">
					...
					<img src="<c:url value='/images/coreframework/com/cmm/btn/btn_del.png' />" class="cursor" onClick="fn_egov_deleteFile('<c:out value="${fileVO.atchFileId}"/>','<c:out value="${fileVO.fileSn}"/>','egov_file_view_table_tr_${status.count}');" alt="<spring:message code="title.attachedFileDelete" />">
				</c:when>
				<c:otherwise>
