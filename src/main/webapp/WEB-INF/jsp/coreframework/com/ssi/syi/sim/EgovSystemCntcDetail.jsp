...
<c:set var="listUri" value="/ssi/syi/sim/getSystemCntcList.cm" />
<c:set var="confirmTF" value="F"/>

/*
 * 관리자로 접근시 처리
 */
<c:if test="${selfUri eq '/ssi/syi/scm/getConfirmSystemCntcDetail.cm'}">
	<c:set var="listUri" value="/ssi/syi/scm/getConfirmSystemCntcList.cm" />
 ...
 * 업무사용자로 접근시 처리
 */
<c:if test="${selfUri eq '/ssi/syi/sim/getSystemCntcDetail.cm'}">
/* ********************************************************
 * 수정화면으로  바로가기
 ******************************************************** */
function fn_egov_modify_SystemCntc(){
	var varForm				    = document.all["Form"];
	varForm.action              = "<c:url value='/ssi/syi/sim/updateSystemCntc.cm'/>";
	varForm.cntcId.value		= "<c:out value='${result.cntcId}'/>";
	varForm.submit();
}
/* ********************************************************
...
function fn_egov_delete_SystemCntc(){
	if (confirm("<spring:message code="common.delete.msg" />")) {
		var varForm				    = document.all["Form"];
		varForm.action			    = "<c:url value='/ssi/syi/sim/removeSystemCntc.cm'/>";
		varForm.cntcId.value		= "<c:out value='${result.cntcId}'/>";
		varForm.submit();
	}
}
	...
	<div class="btn">
		...
		<c:if test="${confirmTF eq 'F'}">
			...
			<c:if test="${result.confmAt eq 'N'}">
				<form name="formUpdt" action="<c:url value='/ssi/syi/sim/updateSystemCntc.cm'/>" method="post" style="display:inline-block; vertical-align:top">
				...
				<form name="formDelete" action="<c:url value='/ssi/syi/sim/removeSystemCntc.cm'/>" method="post" style="display:inline-block; vertical-align:top">
