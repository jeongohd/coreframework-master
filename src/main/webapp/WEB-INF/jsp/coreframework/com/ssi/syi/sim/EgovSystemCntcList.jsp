...
<c:set var="detailUri" value="/ssi/syi/sim/getSystemCntcDetail.cm" />
<c:set var="confirmTF" value="F"/>

/*
 * 관리자 모드 처리
 */
<c:if test="${selfUri eq '/ssi/syi/scm/getConfirmSystemCntcList.cm'}">
	<c:set var="confirmTF" value="T"/>
	<c:set var="detailUri" value="/ssi/syi/scm/getConfirmSystemCntcDetail.cm" />
</c:if>

/*
 * 업무사용자 모드 처리
 */
<c:if test="${selfUri eq '/ssi/syi/sim/getSystemCntcList.cm'}">

/* ********************************************************
 * 등록 처리 함수
 ******************************************************** */
function fn_egov_regist_SystemCntc(){
	location.href = "<c:url value='/ssi/syi/sim/addSystemCntc.cm'/>";
}
/* ********************************************************
 * 수정 처리 함수
 ******************************************************** */
function fn_egov_modify_SystemCntc(){
	location.href = "<c:url value='/ssi/syi/sim/updateSystemCntc.cm'/>";
}
</c:if>
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/ssi/syi/sim/addSystemCntc.cm?pageIndex='/><c:out value='${searchVO.pageIndex}'/>" onclick="" title="<spring:message code="title.create"/>"><spring:message code="button.create"/></a></span>
				</c:if>
			</li>
		</ul>
	</div>
