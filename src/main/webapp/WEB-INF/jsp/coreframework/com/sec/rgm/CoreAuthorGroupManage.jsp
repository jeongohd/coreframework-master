...
function fncSelectAuthorGroupList(pageNo){
    ...
    document.listForm.action = "<c:url value='/sec/rgm/CoreAuthorGroupList.cm'/>";
...
function fncAddAuthorGroupInsert() {
    ...
    if(confirm("<spring:message code="comCopSecRgm.list.validate.confirm.regist" />")) {//등록하시겠습니까?
        ...
        document.listForm.action = "<c:url value='/sec/rgm/CoreAuthorGroupInsert.cm'/>";
        document.listForm.submit();
    }
}
...
function fncAuthorGroupDeleteList() {
    ...
    if(confirm("<spring:message code="comCopSecRgm.list.validate.confirm.delete" />")) { //삭제하시겠습니까?
        document.listForm.action = "<c:url value='/sec/rgm/CoreAuthorGroupDelete.cm'/>";
        document.listForm.submit();
    }
}
...
function fncSelectAuthorGroupPop() {
    ...
    if(document.listForm.searchCondition.value == '3') {
    	window.open("<c:url value='/sec/gmt/CoreGroupSearchList.cm'/>","notice","height=500, width=485, top=50, left=20, scrollbars=no, resizable=no");
    } else {
    ...
    /*
    var url = "<c:url value='/sec/gmt/CoreGroupSearchView.cm'/>";
...
<form:form name="listForm" action="${pageContext.request.contextPath}/sec/ram/CoreAuthorGroupList.cm" method="post">
<div class="board" >
	...
	<tr>
		...
		<td><c:out value="${authorGroup.groupId}"/></td>
