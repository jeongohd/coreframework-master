<%
 /**
  * @Class Name : CoreGroupManage.java
  * @Description : CoreGroupManage List 화면
...
function fncSelectGroupList(pageNo){
    ...
    document.listForm.action = "<c:url value='/sec/gmt/CoreGroupList.cm'/>";
    document.listForm.submit();
}

function fncSelectGroup(groupId) {
    document.listForm.groupId.value = groupId;
    document.listForm.action = "<c:url value='/sec/gmt/CoreGroup.cm'/>";
    document.listForm.submit();
}

function fncAddGroupInsert() {
    location.replace("<c:url value='/sec/gmt/CoreGroupInsertView.cm'/>");
}

function fncGroupListDelete() {
	if(fncManageChecked()) {
	    if(confirm("<spring:message code="comCopSecGmt.validate.confirm.delete" />")) {//삭제하시겠습니까?
            document.listForm.action = "<c:url value='/sec/gmt/CoreGroupListDelete.cm'/>";
            document.listForm.submit();
	    }
	}
}
...
<form:form name="listForm" action="${pageContext.request.contextPath}/sec/gmt/CoreGroupList.cm" method="post">
<div class="board">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
		<ul>
			...
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/sec/gmt/CoreGroupInsertView.cm'/>" onClick="javascript:fncAddGroupInsert();"  title="<spring:message code="button.create" /> <spring:message code="input.button" />"><spring:message code="button.create" /></a></span><!-- 등록 -->
			</li>
		</ul>
	</div>
	...
	<tr>
	    ...
	    <td><a href="<c:url value='/sec/gmt/CoreGroup.cm?groupId=${group.groupId}'/>" onclick="javascript:fncSelectGroup('<c:out value="${group.groupId}"/>')"><c:out value="${group.groupId}"/></a></td>
	    <td><c:out value="${group.groupNm}"/></td>
	    <td><c:out value="${group.groupDc}"/></td>
	    <td><c:out value="${fn:substring(group.groupCreatDe,0,10)}"/></td>
	    <td><a href="<c:url value='/sec/gmt/CoreGroup.cm?groupId=${group.groupId}'/>" onclick="javascript:fncSelectGroup('<c:out value="${group.groupId}"/>')"><img src="<c:url value='/images/coreframework/com/cmm/btn/btn_search.gif'/>"  align="middle" alt="<spring:message code="title.detail" />"  title="<spring:message code="title.detail" />"></a></td>
	</tr>
	</c:forEach>
	</tbody>
	</table>
