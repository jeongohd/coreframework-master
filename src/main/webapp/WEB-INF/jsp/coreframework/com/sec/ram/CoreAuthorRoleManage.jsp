 ...
 * @Description : CoreAuthorRoleManage.jsp
...
function fncSelectAuthorList(){
    ...
    //document.listForm.submit();
    location.href = "<c:url value='/sec/ram/CoreAuthorList.cm'/>";
}

function fncSelectAuthorRole(roleCode) {
    document.listForm.roleCode.value = roleCode;
    document.listForm.action = "<c:url value='/sec/ram/CoreRole.cm'/>";
    document.listForm.submit();
}
...
function fncAddAuthorRoleInsert() {
	if(fncManageChecked()) {
	    if(confirm("<spring:message code="comCopSecRam.authorRoleList.validate.confirm.regist" />")) {//등록하시겠습니까?
            document.listForm.action = "<c:url value='/sec/ram/CoreAuthorRoleInsert.cm'/>";
            document.listForm.submit();
	    }
	} else return;
}
...
<body>
<div class="board">
<form:form name="listForm" action="${pageContext.request.contextPath}/sec/ram/CoreAuthorRoleList.cm" method="post">
	...
	<!-- 
	<div class="btn">
		<span class="btn_s"><a href="<c:url value='/sec/ram/CoreAuthorList.cm'/>"  title="<spring:message code="button.list" /> <spring:message code="input.button" />"><spring:message code="button.list" /></a></span>
