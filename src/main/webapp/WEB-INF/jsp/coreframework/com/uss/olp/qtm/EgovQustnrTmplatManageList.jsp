...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageList.cm'/>";
...
function fn_egov_regist_QustnrTmplatManage(){
	location.href = "<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageRegist.cm' />";
}
/* ********************************************************
 * 수정 처리 함수
 ******************************************************** */
function fn_egov_modify_QustnrTmplatManage(){
	location.href = "<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageModify.cm' />";
}
/* ********************************************************
 * 상세회면 처리 함수
 ******************************************************** */
function fn_egov_detail_QustnrTmplatManage(qestnrTmplatId){
	...
	vFrom.action = "<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageDetail.cm' />";
	vFrom.submit();
}
/* ********************************************************
 * 삭제 처리 함수
...
function fn_egov_delete_QustnrTmplatManage(mtgId){
	...
	if(confirm("<spring:message code='common.delete.msg'/>")){ //삭제 하시겠습니까?
		...
		vFrom.cmd.value = 'del';
		vFrom.action = "<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageList.cm' />";
		vFrom.submit();
	}else{
		vFrom.cmd.value = '';
	}
...
<form name="listForm" action="<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageList.cm'/>" method="post" onSubmit="fn_egov_search_QustnrTmplatManage(); return false;">
	...
	<div class="search_box" title="<spring:message code='common.searchCondition.msg' />">
		<ul>
			...
			<li>
				...
				<!-- 등록버튼 -->
				<span class="btn_b"><a href="<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageRegist.cm' />"  title="<spring:message code='button.create' /> <spring:message code='input.button' />"><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
...
<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>"/>
	...
	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
	  <tr>
		...
		<!-- 템플릿유형 이미지 정보 -->
		<td class="lt_text3"><img src="<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageImg.cm'/>?qestnrTmplatId=${resultInfo.qestnrTmplatId}" alt="${resultInfo.qestnrTmplatTy}<spring:message code='comUssOlpQtm.regist.qestnrTmplatImage' />" title="${resultInfo.qestnrTmplatTy}<spring:message code='comUssOlpQtm.regist.qestnrTmplatImage' />"></td>
		<!-- onLoad="if(this.width>65){this.width=65}" -->
		<!-- 템플릿 설명 -->
		<td class="lt_text3L">
		<a href="<c:url value='/uss/olp/qtm/EgovQustnrTmplatManageDetail.cm'/>?qestnrTmplatId=${resultInfo.qestnrTmplatId}" onClick="fn_egov_detail_QustnrTmplatManage('<c:out value="${resultInfo.qestnrTmplatId}"/>');return false;"><c:out value='${resultInfo.qestnrTmplatCn}'/></a>
		</td>
		<!-- 작성자명 -->
