...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/uss/olp/qmc/EgovQustnrManageList.cm'/>";
...
function fn_egov_regist_QustnrManage(){
	location.href = "<c:url value='/uss/olp/qmc/EgovQustnrManageRegist.cm' />";
}
/* ********************************************************
 * 수정 처리 함수
 ******************************************************** */
function fn_egov_modify_QustnrManage(){
	location.href = "<c:url value='/uss/olp/qmc/EgovQustnrManageModify.cm' />";
}
/* ********************************************************
 * 상세회면 처리 함수
 ******************************************************** */
function fn_egov_detail_QustnrManage(qestnrId){
	...
	vFrom.action = "<c:url value='/uss/olp/qmc/EgovQustnrManageDetail.cm' />";
	vFrom.submit();
}
/* ********************************************************
 * 삭제 처리 함수
...
function fn_egov_delete_QustnrManage(qestnrId){
	...
	if(confirm("<spring:message code='common.delete.msg'/> ")){ //삭제 하시겠습니까?
		vFrom.qestnrId.value = qestnrId;
		vFrom.cmd.value = 'del';
		vFrom.action = "<c:url value='/uss/olp/qmc/EgovQustnrManageList.cm' />";
		vFrom.submit();
	}else{
		vFrom.cmd.value = '';
	}
...
function fn_egov_list_QustnrQestnManag(qestnrId, qestnrTmplatId, Type){
	...
	//QRM QQM QRI
	if(Type == 'QRM'){ //설문응답자정보
		sAction  = "<c:url value='/uss/olp/qrm/EgovQustnrRespondManageList.cm' />";
	}else if(Type == 'QQM'){ //설문문항
		sAction  = "<c:url value='/uss/olp/qqm/EgovQustnrQestnManageList.cm' />";
	}else if(Type == 'QRI'){ //응답자결과
		sAction  = "<c:url value='/uss/olp/qri/EgovQustnrRespondInfoList.cm' />";
	}
...
function fn_egov_statistics_QustnrQestnManag(qestnrId, qestnrTmplatId){
	...
	vFrom.searchCondition.options[0].selected = true;
	vFrom.searchKeyword.value = '';
	vFrom.action = "<c:url value='/uss/olp/qnn/EgovQustnrRespondInfoManageStatistics.cm' />";
	vFrom.submit();
}
</script>
</head>
...
<form name="listForm" action="<c:url value='/uss/olp/qmc/EgovQustnrManageList.cm'/>" method="post" onSubmit="fn_egov_search_QustnrManage(); return false;">
	...
	<div class="search_box" title="<spring:message code='common.searchCondition.msg' />">
		<ul>
			...
			<li>
				...
				<!-- 등록버튼 -->
				<span class="btn_b"> <a href="<c:url value='/uss/olp/qmc/EgovQustnrManageRegist.cm'/>" title="<spring:message code='button.create' /> <spring:message code='input.button' />"><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
...
<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">
	...
	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
	  <tr>
		...
		<!-- 설문제목  -->
		<td class="lt_text3L">
			<a href="<c:url value='/uss/olp/qmc/EgovQustnrManageDetail.cm'/>?qestnrId=${resultInfo.qestnrId}" onClick="fn_egov_detail_QustnrManage('<c:out value="${resultInfo.qestnrId}"/>');return false;"><c:out value='${resultInfo.qestnrSj}'/></a>
    	</td>
		<!-- 설문기간 -->
		...
		<!-- 설문응답자 정보 -->
		<td class="lt_text3">
			<form name="subForm" method="post" action="<c:url value='/uss/olp/qrm/EgovQustnrRespondManageList.cm'/>">
		...
		<!-- 설문문항 -->
		<td class="lt_text3">
			<form name="subForm" method="post" action="<c:url value='/uss/olp/qqm/EgovQustnrQestnManageList.cm'/>">
				<input name="qestnrId" type="hidden" value="${resultInfo.qestnrId}">
				<input name="qestnrTmplatId" type="hidden" value="${resultInfo.qestnrTmplatId}">
				<input name="searchMode" type="hidden" value="Y">
		...
		<!-- 설문조사 -->
		<td class="lt_text3">
			<form name="subForm" method="post" action="<c:url value='/uss/olp/qri/EgovQustnrRespondInfoList.cm'/>">
				<input name="qestnrId" type="hidden" value="${resultInfo.qestnrId}">
				<input name="qestnrTmplatId" type="hidden" value="${resultInfo.qestnrTmplatId}">
				<input name="searchMode" type="hidden" value="Y">
	  	...
	  	<!-- 통계  -->
	  	<td class="lt_text3">
			<form name="subForm" method="post" action="<c:url value='/uss/olp/qnn/EgovQustnrRespondInfoManageStatistics.cm'/>">
				<input name="qestnrId" type="hidden" value="${resultInfo.qestnrId}">
				<input name="qestnrTmplatId" type="hidden" value="${resultInfo.qestnrTmplatId}">
				<input name="searchMode" type="hidden" value="">
