...
function fncSelectMainImageList(pageNo){
    ...
    document.listForm.action = "<c:url value='/uss/ion/msi/selectMainImageList.cm'/>";
    document.listForm.submit();
}

function fncSelectMainImage(imageId) {
    document.listForm.imageId.value = imageId;
    document.listForm.action = "<c:url value='/uss/ion/msi/getMainImage.cm'/>";
    document.listForm.submit();
}
...
function fncAddMainImageInsert() {
    ...
    document.listForm.action = "<c:url value='/uss/ion/msi/addViewMainImage.cm'/>";
    document.listForm.submit();
}
...
function fncLoginMainImageListDelete() {
	if(fncManageChecked()) {
        if(confirm("<spring:message code="uss.ion.msi.mainImageList.deleteImage" />")) {/* 삭제하겠습니까? */
            document.listForm.action = "<c:url value='/uss/ion/msi/removeMainImageList.cm'/>";
            document.listForm.submit();
        }
    }
}
...
<div class="board">
<form name="listForm" action="<c:url value='/uss/ion/msi/selectMainImageList.cm'/>" method="post">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보에 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/uss/ion/msi/addViewMainImage.cm'/>?pageIndex=<c:out value='${mainImageVO.pageIndex}'/>&amp;searchKeyword=<c:out value="${mainImageVO.searchKeyword}"/>&amp;searchCondition=1" onclick="fncAddMainImageInsert(); return false;" title='<spring:message code="button.create" />'><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
...
<input type="hidden" name="searchCondition" value="1" >
	...
	<table class="board_list">
		...
		<tbody>
			...
			<tr>
				<td>
					<form name="item" method="post" action="<c:url value='/uss/ion/msi/getMainImage.cm'/>">
