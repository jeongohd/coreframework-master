...
function fncSelectUserAbsnceList(pageNo) {
    ...
    document.listForm.action = "<c:url value='/uss/ion/uas/selectUserAbsnceList.cm'/>";
...
function fncSelectUserAbsnce(userId, regYn) {
	if(regYn == 'N') {
        if(confirm("<spring:message code="ussIonUas.userAbsnceList.goToregisPage" />")) {/* 등록된 사용자부재 정보가 없습니다. 등록페이지로 이동하시겠습니까? */
        	location.replace("<c:url value='/uss/ion/uas/addViewUserAbsnce.cm'/>?userId="+userId);
        } else {
            return;
        }
	}
    document.listForm.userId.value = userId;
    document.listForm.action = "<c:url value='/uss/ion/uas/getUserAbsnce.cm'/>";
    document.listForm.submit();
}

function fncAddUserAbsnceInsert() {
	if(fncInsertCheckId()) {
        document.listForm.action = "<c:url value='/uss/ion/uas/addViewUserAbsnce.cm'/>";
        document.listForm.submit();
	}
}
...
function fncLoginUserAbsnceListDelete() {
	if(fncManageChecked()) {
        if(confirm("<spring:message code="ussIonUas.userAbsnceList.deleteMsg" />")) {/* 삭제하시겠습니까? */
            document.listForm.action = "<c:url value='/uss/ion/uas/removeUserAbsnceList.cm'/>";
            document.listForm.submit();
        }
    }
}
...
<div class="board">
<form name="listForm" action="<c:url value='/uss/ion/uas/selectUserAbsnceList.cm'/>" method="post">
		...
		<tbody>
			...
			<td colspan="5">
			 ...
			 <c:forEach var="userAbsnce" items="${userAbsnceList}" varStatus="status">
			  <tr>
			    <td>
			        <form name="item" method="post" action="<c:url value='/uss/ion/uas/getUserAbsnce.cm'/>">
