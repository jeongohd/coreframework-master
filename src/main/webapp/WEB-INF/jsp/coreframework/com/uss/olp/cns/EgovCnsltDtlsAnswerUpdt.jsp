...
function fn_egov_updt_cnsltdtlsanswer(form, cnsltId){
	...
	// 서버사이드 테스트용
	/*
		form.action = "<c:url value='/uss/olh/wor/WordDicaryRegist.cm'/>";
		form.submit();
		return;
	*/
	...
	if (!validateCnsltManageVO(form)) {
		...
		form.action = "<c:url value='/uss/olp/cnm/CnsltDtlsAnswerUpdt.cm'/>";
		form.submit();
...
function fn_egov_inqire_qnaanswerlist() {
	...
	cnsltManageVO.action = "<c:url value='/uss/olp/cnm/CnsltAnswerListInqire.cm'/>";
...
function fn_egov_pop_mailform() {
	...
	var url 	= "<c:url value='/ems/insertSndngMailView.cm'/>";
...
<div class="wTableFrm">
<!-- 상단타이틀 -->
<form:form modelAttribute="cnsltManageVO" action="${pageContext.request.contextPath}/uss/olp/cnm/CnsltDtlsAnswerUpdt.cm" method="post">
		...
		<tr>
			...
			<td class="nopd">
				...
				<input name="emailAnswerAt" type="checkbox" disabled value="Y" <c:if test="${result.emailAnswerAt == 'Y'}">checked</c:if> title="<spring:message code="comUssOlpCns.regist.emailAt"/>"><spring:message code="comUssOlpCns.regist.emailAt"/><!-- 이메일답변여부 -->
  			    ...
  			    &nbsp;&nbsp;&nbsp;
				<a href = "<c:url value='/images/coreframework/com/cmm/icon/search2.gif'/>" onClick="javascript:fn_egov_pop_mailform(); return false;" alt="발송메일 등록 팝업"></a>
 				</c:if>
			</td>
		</tr>
		<!-- 상담제목 -->
...
<div class="btn">
	...
	<span class="btn_s"><a href="<c:url value='/uss/olp/cnm/CnsltAnswerListInqire.cm' />" onclick="fn_egov_inqire_qnaanswerlist(); return false;"  title="<spring:message code="button.list" />  <spring:message code="input.button" />"><spring:message code="button.list" /></a></span>
