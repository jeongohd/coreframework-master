...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/uss/sam/ipm/listIndvdlInfoPolicy.cm'/>";
...
function fn_egov_detail_IndvdlInfoPolicy(indvdlInfoPolicyId){
	...
	vFrom.action = "<c:url value='/uss/sam/ipm/detailIndvdlInfoPolicy.cm' />";
	vFrom.submit();
}
...
function fn_egov_search_IndvdlInfoPolicy(){
	...
	vFrom.action = "<c:url value='/uss/sam/ipm/listIndvdlInfoPolicy.cm' />";
	vFrom.submit();
...
<div class="board">
	...
	<form name="listForm" action="<c:url value='/uss/sam/ipm/listIndvdlInfoPolicy.cm'/>" method="post">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다 -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/uss/sam/ipm/registIndvdlInfoPolicy.cm' />" onclick="fn_userChk('USRCNFRM_00000000000')"><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
		...
		<tbody>
			...
			<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
				<tr>
					...
					<td>			
						<a href="<c:url value='/uss/sam/ipm/detailIndvdlInfoPolicy.cm'/>?indvdlInfoId=${resultInfo.indvdlInfoPolicyId}&pageIndex=${searchVO.pageIndex}" onClick="fn_egov_detail_IndvdlInfoPolicy('<c:out value="${resultInfo.indvdlInfoPolicyId}"/>');return false;"><c:out value='${resultInfo.indvdlInfoPolicyNm}'/></a>
					</td>
