...
<!--
	...
	function fncEgovCtsnnManageList(){
		location.href = "<c:url value='/uss/ion/ctn/selectCtsnnManageList.cm'/>";
	}
<c:if test="${ctsnnManageVO.confmAt eq 'A' }">	
	...
	******************************************************** */
	function fncEgovCtsnnManage() {
			...
			varFrom.action = "<c:url value='/uss/ion/ctn/EgovCtsnnManageDetail.cm'/>";
			varFrom.submit();   
	}
	...
	******************************************************** */
	function fncDeleteCtsnnManage() {
		    var varFrom = document.getElementById("ctsnnManage");
		    varFrom.action = "<c:url value='/uss/ion/ctn/deleteCtsnnManage.cm'/>";
...
<input type="hidden" name="cmd"        value="updt" />
	...
	<!-- 결재권자 정보 Include -->
	<c:import url="/uss/ion/ism/selectInfrmlSanctn.cm" charEncoding="utf-8">
	...
	<div class="btn">
		<c:if test="${ctsnnManageVO.confmAt eq 'A' }">
			<span class="btn_s"><a href="<c:url value='/uss/ion/ctn/EgovCtsnnManageDetail.cm'/>?cmd=updt&usid=<c:out value='${ctsnnManageVO.usid}'/>&ctsnnCd=<c:out value='${ctsnnManageVO.ctsnnCd}'/>&reqstDe=<c:out value='${ctsnnManageVO.reqstDe}'/>" onclick="fncEgovCtsnnManage(); return false;"><spring:message code="button.update" /></a></span>
          	<span class="btn_s"><a href="<c:url value='/uss/ion/ctn/deleteCtsnnManage.cm'/>?cmd=delete&usid=<c:out value='${ctsnnManageVO.usid}'/>&ctsnnCd=<c:out value='${ctsnnManageVO.ctsnnCd}'/>&reqstDe=<c:out value='${ctsnnManageVO.reqstDe}'/>&infrmlSanctnId=<c:out value='${ctsnnManageVO.infrmlSanctnId}'/>" onclick="fncDeleteCtsnnManage(); return false;"><spring:message code="button.delete" /></a></span>
        </c:if>
         
		<span class="btn_s"><a href="<c:url value='/uss/ion/ctn/selectCtsnnManageList.cm'/>?searchCondition=1" onclick="fncEgovCtsnnManageList(); return false;"><spring:message code="button.list" /></a></span>
	</div>
