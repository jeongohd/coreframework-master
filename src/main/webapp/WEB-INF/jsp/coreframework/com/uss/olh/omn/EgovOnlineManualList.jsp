...
<c:set var="ImgUrl" value="${pageContext.request.contextPath}/images/coreframework/com/cmm/"/>
<c:set var="CssUrl" value="${pageContext.request.contextPath}/css/coreframework/com/" />
<!DOCTYPE html>
<head>
<title>온라인매뉴얼</title>
...
<%-- 온라인 도움말 설정 --%>
<c:import charEncoding="utf-8" url="/uss/olh/omn/setOnlineManual.cm" >
...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/uss/olh/omn/listOnlineManual.cm'/>";
...
function fn_egov_search_OnlineManual(){
	...
	vFrom.action = "<c:url value='/uss/olh/omn/listOnlineManual.cm'/>";
	vFrom.submit();
...
<!--  상단타이틀 -->
<form name="listForm" action="<c:url value='/uss/olh/omn/listOnlineManual.cm'/>" method="post">
<%-- noscript 테그 --%>
...
<tr>
    ...
    <td class="lt_text3L" nowrap>
    	<form name="subForm" method="post" action="<c:url value='/uss/olh/omn/detailOnlineManual.cm'/>">
