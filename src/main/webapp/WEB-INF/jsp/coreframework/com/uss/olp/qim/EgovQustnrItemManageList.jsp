...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/uss/olp/qim/EgovQustnrItemManageList.cm'/>";
...
function fn_egov_regist_QustnrItemManage(){
	location.href = "<c:url value='/uss/olp/qim/EgovQustnrItemManageRegist.cm' />";
}
/* ********************************************************
 * 수정 처리 함수
 ******************************************************** */
function fn_egov_modify_QustnrItemManage(){
	location.href = "<c:url value='/uss/olp/qim/EgovQustnrItemManageModify.cm' />";
}
/* ********************************************************
 * 상세회면 처리 함수
 ******************************************************** */
function fn_egov_detail_QustnrItemManage(qustnrIemId){
	...
	vFrom.action = "<c:url value='/uss/olp/qim/EgovQustnrItemManageDetail.cm' />";
	vFrom.submit();
}
/* ********************************************************
 * 삭제 처리 함수
...
function fn_egov_delete_QustnrItemManage(qestnrId){
	...
	if(confirm("<spring:message code='common.delete.msg'/>")){ //삭제 하시겠습니까?
		...
		vFrom.cmd.value = 'del';
		vFrom.action = "<c:url value='/uss/olp/qim/EgovQustnrItemManageList.cm' />";
		vFrom.submit();
	}else{
		vFrom.cmd.value = '';
	}
...
<form name="listForm" action="<c:url value='/uss/olp/qim/EgovQustnrItemManageList.cm'/>" method="post" onSubmit="fn_egov_search_QustnrQestnManage(); return false;">
	...
	<div class="search_box" title="<spring:message code='common.searchCondition.msg' />">
		<ul>
			...
			<li>
				...
				<!-- 등록버튼 -->
				<span class="btn_b"> <a href="<c:url value='/uss/olp/qim/EgovQustnrItemManageRegist.cm'/>" title="<spring:message code='button.create' /> <spring:message code='input.button' />"><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
	...
	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
	  <tr>
		...
		<!-- 항목내용  -->
		<td class="lt_text3L">
		<a href="<c:url value='/uss/olp/qim/EgovQustnrItemManageDetail.cm'/>?qustnrIemId=${resultInfo.qustnrIemId}" onClick="fn_egov_detail_QustnrItemManage('<c:out value="${resultInfo.qustnrIemId}"/>');return false;"><c:out value='${resultInfo.iemCn}'/></a>
		</td>
		<!-- 기타답변여부 -->
