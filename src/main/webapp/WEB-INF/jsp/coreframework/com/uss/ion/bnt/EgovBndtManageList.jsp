...
<%@ page import="coreframework.com.uss.ion.bnt.service.BndtManageVO"%>
...
<!--
	...
	function fncSelectBndtManageList(){
	    document.location.href = "<c:url value='/uss/ion/bnt/EgovBndtManageList.cm'/>?year=<%=year%>&amp;month=<%=month%>";
	}
	...
	function fncSelectBndtManage(bndtDe,bndtId) {
	    ...
	    document.listForm.action = "<c:url value='/uss/ion/bnt/EgovBndtManageDetail.cm'/>";
	    document.listForm.submit();
	}
	/*설명 : 당직 신규등록 화면 호출 */
	function fncInsertBndtManage(bndtDe) {
	    document.listForm.bndtDe.value = bndtDe;
	    document.listForm.action = "<c:url value='/uss/ion/bnt/EgovBndtManageRegist.cm'/>";
	    document.listForm.submit();
	}
	...
	function fncSelectBndtDiary(bndtDe,bndtId, cmd) {
		...
		document.listForm.bndtId.value = bndtId;
	    document.listForm.bndtDe.value = bndtDe;
	    document.listForm.action = "<c:url value='/uss/ion/bnt/selectBndtDiary.cm'/>";
	    document.listForm.submit();
	}
	...
	function fncBndtManageBnde() {
		...
		varForm.method = "post";
		varForm.action = "<c:url value='/uss/ion/bnt/EgovBndtManageListPop.cm'/>";
...
<div id="border" style="width:712px">
	...
	<div class="search_box2 mb10" style="text-align:center">
		...
		<a href="#LINK" onclick="location.href='<c:url value='/uss/ion/bnt/EgovBndtManageList.cm'/>?year=<%=year-1%>&amp;month=<%=month%>'" style="margin-left:20px; vertical-align:-4px"><img src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png'/>" alt="<spring:message code="comUssIonBnt.common.year.prev"/>" style="margin-right:4px;border:0px;"></a><!-- 연도 이전 -->
		<span class="t1"><%=year%><spring:message code="comUssIonBnt.common.year"/></span><!-- 년 -->
		<a href="#LINK" onclick="location.href='<c:url value='/uss/ion/bnt/EgovBndtManageList.cm'/>?year=<%=year+1%>&amp;month=<%=month%>'" style="margin-right:20px; vertical-align:-4px"><img src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png'/>"  alt="<spring:message code="comUssIonBnt.common.year.next"/>" style="margin-left:4px;border:0px;"></a><!-- 연도 이후 -->
		
		<%if(month > 0 ){ %>
		<a href="#LINK" onclick="location.href='<c:url value='/uss/ion/bnt/EgovBndtManageList.cm'/>?year=<%=year%>&amp;month=<%=month-1%>'" style="vertical-align:-4px"><img src="<c:url value='/images/coreframework/com/cmm/icon/icon_prev.png'/>" alt="<spring:message code="comUssIonBnt.common.month.prev"/>" style="margin-right:4px;"></a><!-- 월 이전 -->
		<%}%>
		<span class="t1"><%=month+1%><spring:message code="comUssIonBnt.common.month"/></span><!-- 월 -->
		<%if(month < 11 ){ %>
		<a href="#LINK" onclick="location.href='<c:url value='/uss/ion/bnt/EgovBndtManageList.cm'/>?year=<%=year%>&amp;month=<%=month+1%>'" style="margin-right:20px; vertical-align:-4px"><img src="<c:url value='/images/coreframework/com/cmm/icon/icon_next.png'/>"  alt="<spring:message code="comUssIonBnt.common.month.next"/>" style="margin-left:4px;"></a><!-- 월 이후 -->
		<%}%>
		
		<a class="btn_01" href="<c:url value='/uss/ion/bnt/EgovBndtManageListPop.cm'/>" onclick="fncBndtManageBnde(); return false;" target="_blank" title="<spring:message code="comUssIonBnt.common.toNewWindow"/>"><spring:message code="comUssIonBnt.bndtManageList.bndtExcelRegist"/></a><!-- 새 창으로 이동 --><!-- 당직엑셀등록 -->
	</div>
