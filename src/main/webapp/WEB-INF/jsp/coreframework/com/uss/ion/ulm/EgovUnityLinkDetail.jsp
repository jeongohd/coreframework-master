...
function fn_egov_list_UnityLink(){
	location.href = "<c:url value='/uss/ion/ulm/listUnityLink.cm' />";
}
/* ********************************************************
 * 저장처리화면
 ******************************************************** */
function fn_egov_modify_UnityLink(){
	...
	vFrom.cmd.value = '';
	vFrom.action = "<c:url value='/uss/ion/ulm/updtUnityLink.cm' />";;
	vFrom.submit();
...
function fn_egov_delete_UnityLink(){
	...
	if(confirm("<spring:message code="uss.ion.ulm.unityLinkDetail.validate.deleteMsg"/>")){/* 삭제 하시겠습니까? */
		vFrom.cmd.value = 'del';
		vFrom.action = "<c:url value='/uss/ion/ulm/detailUnityLink.cm' />";
		vFrom.submit();
	}else{
		vFrom.cmd.value = '';
	}
...
<input name="cmd" type="hidden" value="<c:out value=''/>"/>
	...
	<!-- 하단 버튼 -->
	<div class="btn">
		<form name="formUpdt" action="<c:url value='/uss/ion/ulm/updtUnityLink.cm'/>" method="post" style="display:inline-block; vertical-align:top">
			<input name="unityLinkId" type="hidden" value="${unityLink.unityLinkId}">
			<input class="s_submit" type="submit" value="<spring:message code="button.update" />" onclick="fn_egov_modify_UnityLink(); return false;">
		</form>
		
		<form name="formDelete" action="<c:url value='/uss/ion/ulm/detailUnityLink.cm'/>" method="post" style="display:inline-block; vertical-align:top">
		<input name="unityLinkId" type="hidden" value="${unityLink.unityLinkId}">
		...
		<form name="formList" action="<c:url value='/uss/ion/ulm/listUnityLink.cm'/>" method="post" style="display:inline-block; vertical-align:top">
