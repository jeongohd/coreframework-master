...
function linkPage(pageNo){
	...
	document.listForm.action = "<c:url value='/uss/olp/qqm/EgovQustnrQestnManageList.cm'/>";
...
function fn_egov_regist_QustnrQestnManage(){
	...
	vFrom.cmd.value = '';
	vFrom.action = "<c:url value='/uss/olp/qqm/EgovQustnrQestnManageRegist.cm' />";
	vFrom.submit();
}
/* ********************************************************
 * 수정 처리 함수
...
function fn_egov_modify_QustnrQestnManage(){
	location.href = "<c:url value='/uss/olp/qqm/EgovQustnrQestnManageModify.cm' />";
}
/* ********************************************************
 * 상세회면 처리 함수
 ******************************************************** */
function fn_egov_detail_QustnrQestnManage(qestnrQesitmId){
	...
	vFrom.action = "<c:url value='/uss/olp/qqm/EgovQustnrQestnManageDetail.cm' />";
	vFrom.submit();
}
/* ********************************************************
 * 문항통계
...
function fn_egov_statistics_QustnrQestnManage(qestnrQesitmId,qestnTyCode){
	...
	vFrom.action = "<c:url value='/uss/olp/qqm/EgovQustnrQestnManageStatistics.cm' />";
	vFrom.submit();
}
...
function fn_egov_delete_QustnrQestnManage(qestnrId){
	...
	if(confirm("<spring:message code='common.delete.msg'/>")){ //삭제 하시겠습니까?
		...
		vFrom.cmd.value = 'del';
		vFrom.action = "<c:url value='/uss/olp/qqm/EgovQustnrQestnManageList.cm' />";
		vFrom.submit();
	}else{
		vFrom.cmd.value = '';
	}
...
<form name="listForm" action="<c:url value='/uss/olp/qqm/EgovQustnrQestnManageList.cm'/>" method="post" onSubmit="fn_egov_search_QustnrQestnManage(); return false;">
	...
	<div class="search_box" title="<spring:message code='common.searchCondition.msg' />">
		<ul>
			...
			<li>
				...
				<!-- 등록버튼 -->
				<span class="btn_b"> <a href="<c:url value='/uss/olp/qqm/EgovQustnrQestnManageRegist.cm'/>" title="<spring:message code='button.create' /> <spring:message code='input.button' />"><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
...
<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">
	...
	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
	  <tr>
		...
		<!-- 질문내용  -->
		<td class="lt_text3L">
			<a href="<c:url value='/uss/olp/qqm/EgovQustnrQestnManageDetail.cm'/>?qestnrQesitmId=${resultInfo.qestnrQesitmId}" onClick="fn_egov_detail_QustnrQestnManage('<c:out value="${resultInfo.qestnrQesitmId}"/>');return false;"><c:out value='${resultInfo.qestnCn}'/></a>
    	</td>
		<!-- 질문유형 -->
		<td class="lt_text3">
		...
		<!-- 질문항목 -->
		<td class="lt_text3">
			<form name="subForm" method="post" action="<c:url value='/uss/olp/qim/EgovQustnrItemManageList.cm'/>">
		...
		<!-- 통계  -->
	  	<td class="lt_text3">
			<form name="subForm" method="post" action="<c:url value='/uss/olp/qqm/EgovQustnrQestnManageStatistics.cm'/>">
			<c:if test="${qustnrQestnManageVO.searchMode == 'Y'}">
			...
			<!-- EgovQustnrQestnManageStatistics.cm 로 파라미터 값을 두개 전달하기 위해 아래와 같이 input넣고 span도 수정 -->
