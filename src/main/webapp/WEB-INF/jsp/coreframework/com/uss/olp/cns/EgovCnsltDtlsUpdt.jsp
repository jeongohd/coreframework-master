...
function fn_egov_updt_cnsltdtls(form, cnsltId){
		...
		if (!validateCnsltManageVO(form)) {
			...
			form.action = "<c:url value='/uss/olp/cns/CnsltDtlsUpdt.cm'/>";
			form.submit();
...
function fn_egov_inqire_qnalist() {
	...
	cnsltManageVO.action = "<c:url value='/uss/olp/cns/CnsltListInqire.cm'/>";
...
<div class="wTableFrm">
<!-- 상단타이틀 -->
<form:form modelAttribute="cnsltManageVO" action="${pageContext.request.contextPath}/uss/olp/cns/CnsltDtlsUpdt.cm" method="post" enctype="multipart/form-data" onSubmit="fn_egov_updt_cnsltdtls(document.forms[0],'${result.cnsltId}'); return false;">

<!-- CnsltDtlsUpdtView.cm Call을 위한 처리  -->
...
<!-- 첨부파일 삭제 후 리턴 URL -->
<input type="hidden" name="returnUrl" value="<c:url value='/uss/olp/cns/CnsltDtlsUpdtView.cm'/>"/>
...
<div class="btn">
	...
	<span class="btn_s"><a href="<c:url value='/uss/olp/cns/CnsltListInqire.cm' />" onclick="fn_egov_inqire_qnalist(); return false;"  title="<spring:message code="button.list" />  <spring:message code="input.button" />"><spring:message code="button.list" /></a></span>
