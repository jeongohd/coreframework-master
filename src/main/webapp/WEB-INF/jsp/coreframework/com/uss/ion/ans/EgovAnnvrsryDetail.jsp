...
function fncSelectAnnvrsryManageList(pageNo){
    ...
    document.DetailForm.action = "<c:url value='/uss/ion/ans/selectAnnvrsryManageList.cm'/>";
...
function fncSelectAnnvrsryManage() {
    ...
    document.DetailForm.action = "<c:url value='/uss/ion/ans/selectAnnvrsryManage.cm'/>";
    document.DetailForm.submit();   
}
/*설명 : 기념일 삭제처리*/
function fncDeleteAnnvrsry() {
    var varFrom = document.getElementById("deleteForm");
    varFrom.action = "<c:url value='/uss/ion/ans/deleteAnnvrsryManage.cm'/>";
...
<div class="wTableFrm">
	...
	<!-- 하단 버튼 -->
	<div class="btn">
		<form id="selectForm" name="selectForm" action="<c:url value='/uss/ion/ans/selectAnnvrsryManage.cm'/>" method="post" style="display:inline-block; vertical-align:top">
		...
		<form id="deleteForm" name="deleteForm" method="post" action="<c:url value='/uss/ion/ans/deleteAnnvrsryManage.cm'/>" style="display:inline-block; vertical-align:top">
		<input type="hidden" name="annId"   value="<c:out value="${annvrsryManageVO.annId      }"/>">
		<input class="s_submit" type="submit" value='<spring:message code="button.delete" />' onclick="fncDeleteAnnvrsry(); return false;" />
        </form>
        
        <span class="btn_s"><a href="<c:url value='/uss/ion/ans/selectAnnvrsryManageList.cm'/>?searchCondition=1" onclick="fncSelectAnnvrsryManageList(); return false;"><spring:message code="button.list" /></a></span>
	</div>
