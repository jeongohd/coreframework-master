...
function fn_egov_popupOpen_Init(){
	...
	fn_egov_popupOpen_TwitterMain('<c:url value='/uss/ion/tir/selectTwitterPopup.cm'/>?cmd=<c:out value="${sCmd}"/>','TwitterPopupAuth',900,300);
}
...
if( !rtoken.equals("") && !rtoken.equals("") &&
	...
	if(sCmd.equals("RECPTN")){
		%>location.href = '<c:url value='/uss/ion/tir/listTwitterRecptn.cm'/>';<%
	}else{
		%>location.href = '<c:url value='/uss/ion/tir/registTwitterTrnsmit.cm'/>';<%
	}
...
<div class="wTableFrm">
	...
	<ol class="twit_list mt20">
		...
		<img src="<c:url value='/images/coreframework/com/uss/ion/tir/twitter_regist_detail.jpg'/>" alt="트위터 어플 등록 완료 이미지" title="트위터 어플 등록 완료 이미지"></li>
	</ol>
</div>
