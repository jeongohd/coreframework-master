...
<%
	...
	if(sAt.equals("1")){
		//out.println("location.href='/uss/ion/tir/selectTwitterPopup.cm?cmd="+sCmd+"&verify=Y&at=1';");
	%>
		location.href="<c:url value='/uss/ion/tir/selectTwitterPopup.cm'/>?cmd=<c:out value="${sCmd}"/>&verify=Y&at=1";
	<%
	}else{
		//out.println("location.href='/uss/ion/tir/selectTwitterPopup.cm?cmd="+sCmd+"&verify=Y';");
	%>
		location.href="<c:url value='/uss/ion/tir/selectTwitterPopup.cm'/>?cmd=<c:out value="${sCmd}"/>&verify=Y";
	<%
	}
%>
</script>
