...
<%@ page import="coreframework.com.cmm.util.CoreBasicLogger"%>
<%@ page import="coreframework.com.cmm.util.CoreBasicLogger" %>
...
<%
	...
	if (rtoken.equals(oauthToken)) {
		try {
			...
			CoreBasicLogger.debug("Twitter Access Token getting error", te);
		} catch (Exception e) {
			//e.printStackTrace();// KISA 보안취약점 조치 (2018-12-10, 신용호)
			CoreBasicLogger.debug("error", e);
		}
	}
...
opener.location.href = '<c:url value="/uss/ion/tir/listTwitterRecptn.cm"/>';
<%} else {%>
opener.location.href = '<c:url value="/uss/ion/tir/registTwitterTrnsmit.cm"/>';
<%}%>
window.close();
</script>
</head>
