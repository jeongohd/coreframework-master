...
function fncSelectIntnetSvcGuidanceList(pageNo) {
    ...
    document.listForm.action = "<c:url value='/uss/ion/isg/selectIntnetSvcGuidanceList.cm'/>";
    document.listForm.submit();
}

function fncSelectIntnetSvcGuidance(intnetSvcId) {
    document.listForm.intnetSvcId.value = intnetSvcId;
    document.listForm.action = "<c:url value='/uss/ion/isg/getIntnetSvcGuidance.cm'/>";
    document.listForm.submit();
}
...
function fncAddIntnetSvcGuidanceInsert() {
    ...
    document.listForm.action = "<c:url value='/uss/ion/isg/addViewIntnetSvcGuidance.cm'/>";
    document.listForm.submit();
}
...
function fncViewIntnetSvcGuidance() {
	...
	var url = "<c:url value='/uss/ion/isg/selectIntnetSvcGuidanceResultList.cm'/>";
...
<div class="board">
<form name="listForm" action="<c:url value='/uss/ion/isg/selectIntnetSvcGuidanceList.cm'/>" method="post">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/uss/ion/isg/addViewIntnetSvcGuidance.cm'/>?pageIndex=<c:out value='${intnetSvcGuidanceVO.pageIndex}'/>&amp;searchKeyword=<c:out value="${intnetSvcGuidanceVO.searchKeyword}"/>&amp;searchCondition=1" onclick="fncAddIntnetSvcGuidanceInsert(); return false;" title='<spring:message code="button.create" />'><spring:message code="button.create" /></a></span>
				<span class="btn_b"><a href="<c:url value='/uss/ion/isg/selectIntnetSvcGuidanceResultList.cm'/>" onclick="fncViewIntnetSvcGuidance(); return false;" title='새 창으로 이동' target="_blank"><spring:message code="button.confirm" /></a></span>
			</li>
		</ul>
	</div>
		...
		<tbody>
			<%-- 데이터를 없을때 화면에 메세지를 출력해준다 --%>
			 ...
			 <c:forEach var="intnetSvcGuidance" items="${intnetSvcGuidanceList}" varStatus="status">
			  <tr>
			    <td>
			        <form name="item" method="post" action="<c:url value='/uss/ion/isg/getIntnetSvcGuidance.cm'/>">
