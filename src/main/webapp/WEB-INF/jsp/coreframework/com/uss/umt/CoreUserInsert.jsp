...
<script type="text/javascript" src="<c:url value='/js/coreframework/com/sym/ccm/zip/coreZipPopup.js' />" ></script>
...
function fn_modal_setting(){
	...
	//타이틀 설졍
	$("#egovModal").setCoreModalTitle("<spring:message code="comUssUmt.userManageRegistModal.title" />"); //아이디 중복 확인
	var content = "";
	...
	//모달 body 설정
	$("#egovModal").setCoreModalBody(content);
	...
	//모달 footer 설정
	$("#egovModal").setCoreModalfooter(footer);
...
function fn_id_check(){	
	$.ajax({
		type:"POST",
		url:"<c:url value='/uss/umt/coreIdDplctCnfirmAjax.cm' />",
		data:{
...
function fnIdCheck1(){
    var retVal;
    var url = "<c:url value='/uss/umt/coreIdDplctCnfirm.cm'/>";
...
function fnListPage(){
    document.userManageVO.action = "<c:url value='/uss/umt/coreUserManageList.cm'/>";
...
function fn_egov_inqire_cert() {
	var url = "<c:url value='/uat/uia/CoreGpkiRegist.cm' />";
	var popupwidth = '500';
	var popupheight = '400';
	var title = '인증서';
...
<form:form modelAttribute="userManageVO" action="${pageContext.request.contextPath}/uss/umt/coreUserManageAdd.cm" name="userManageVO" method="post" onSubmit="fnInsert(document.forms[0]); return false;">
<div class="wTableFrm">
	<!-- 타이틀 -->
	...
	<!-- 하단 버튼 --> 
	<div class="btn">
		<span class="btn_s"><a href="<c:url value='/uss/umt/coreUserManageList.cm' />"  title="<spring:message code="button.list" />  <spring:message code="input.button" />"><spring:message code="button.list" /></a></span>
...
<!-- 우편번호검색 -->
<input type="hidden" name="zip_url" value="<c:url value='/sym/ccm/zip/CoreCcmZipSearchPopup.cm'/>" />
</form:form>

<!-- Core Modal include  -->
<c:import url="/coreModal.cm" charEncoding="utf-8">
