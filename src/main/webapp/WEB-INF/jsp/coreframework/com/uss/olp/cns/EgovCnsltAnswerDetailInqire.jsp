...
function fn_egov_inqire_cnsltanswerlist() {
	...
	document.CnsltManageForm.action = "<c:url value='/uss/olp/cnm/CnsltAnswerListInqire.cm'/>";
...
function fn_egov_updt_cnsltdtlsanswer(cnsltId){
	...
	document.CnsltManageForm.action = "<c:url value='/uss/olp/cnm/CnsltDtlsAnswerUpdtView.cm'/>";
	document.CnsltManageForm.submit();			
}
...
<div class="wTableFrm">
<!-- 상단타이틀 -->
<form name="CnsltManageForm" action="<c:url value='/uss/olp/cnm/CnsltDtlsAnswerUpdtView.cm'/>" method="post"  onsubmit="fn_egov_updt_cnsltdtlsanswer('<c:out value="${result.cnsltId}"/>'); return false;">
...
<c:if test="${result.qnaProcessSttusCode == '3'}">
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/uss/olp/cnm/CnsltAnswerListInqire.cm' />"  title="<spring:message code="title.list" /> <spring:message code="input.button" />"><spring:message code="button.list" /></a></span><!-- 목록 -->
