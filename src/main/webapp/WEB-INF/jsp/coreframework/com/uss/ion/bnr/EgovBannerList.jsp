...
function fncSelectBannerList(pageNo){
    ...
    document.listForm.action = "<c:url value='/uss/ion/bnr/selectBannerList.cm'/>";
    document.listForm.submit();
}

function fncSelectBanner(bannerId) {
    document.listForm.bannerId.value = bannerId;
    document.listForm.action = "<c:url value='/uss/ion/bnr/getBanner.cm'/>";
    document.listForm.submit();
}
...
function fncAddBannerInsert() {
    ...
    document.listForm.action = "<c:url value='/uss/ion/bnr/addViewBanner.cm'/>";
    document.listForm.submit();
}
...
function fncBannerListDelete() {
	if(fncManageChecked()) {
        if(confirm("<spring:message code="ussIonBnr.bannerList.deleteImage"/>")) {/* 삭제하시겠습니까? */
            document.listForm.action = "<c:url value='/uss/ion/bnr/removeBannerList.cm'/>";
            document.listForm.submit();
        }
    }
}
...
<div class="board">
	...
	<form name="listForm" action="<c:url value='/uss/ion/bnr/selectBannerList.cm'/>" method="post">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg"/>"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/uss/ion/bnr/addViewBanner.cm'/>?pageIndex=<c:out value='${bannerVO.pageIndex}'/>&amp;searchKeyword=<c:out value="${bannerVO.searchKeyword}"/>&amp;searchCondition=1" onclick="fncAddBannerInsert(); return false;" title="<spring:message code="button.create" />"><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
	...
	<table class="board_list">
		...
		<tbody>
			...
			<tr>
				<td>
					<form name="item" method="post" action="<c:url value='/uss/ion/bnr/getBanner.cm'/>">
