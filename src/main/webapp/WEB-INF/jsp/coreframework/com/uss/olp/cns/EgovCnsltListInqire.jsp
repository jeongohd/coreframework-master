...
function fn_egov_select_linkPage(pageNo){
	...
	document.CnsltListForm.action = "<c:url value='/uss/olp/cns/CnsltListInqire.cm'/>";
...
function fn_egov_regist_cnsltdtls(){
	...
	var loginRealnm_url 	= "<c:url value='/uss/olp/cns/LoginRealnmChoice.cm'/>";
	var	loginRealnm_status 	= "dialogWidth=350px;dialogHeight=150px;resizable=no;center=yes";

	// 로그인 화면
	var	login_url 		= "<c:url value='/uat/uia/coreLoginUsr.cm'/>";
	var login_status 	= "dialogWidth=700px;dialogHeight=420px;resizable=no;center=yes";


	// 실명확인 화면
	var	realnm_url 	= "<c:url value='/sec/rnc/EgovRlnmCnfirm.cm?nextUrlName=button.cnsltregist&nextUrl=C'/>";
	 	...
	 	if	(returnValue)	{
	 		...
	 		if (ls_loginRealnmAt == "L")		{
				...
				*/
				...
				// 팝업이 아닌 메인 화면으로 처리.
	 			document.CnsltListForm.action = "<c:url value='/uat/uia/coreLoginUsr.cm'/>";
	 			document.CnsltListForm.submit();
...
function fn_egov_regist_cnsltcn(){
	...
	document.CnsltListForm.action = "<c:url value='/uss/olp/cns/CnsltDtlsRegistView.cm'/>";
	document.CnsltListForm.submit();
...
//	document.CnsltListForm.action = "<c:url value='/uss/olp/cns/CnsltDetailInqire.cm'/>";
	document.CnsltListForm.action = "<c:url value='/uss/olp/cns/CnsltInqireCoUpdt.cm'/>";
  	document.CnsltListForm.submit();
...
<div class="board">
<form name="CnsltListForm" action="<c:url value='/uss/olp/cns/CnsltListInqire.cm'/>" method="post" onSubmit="fn_egov_search_cnsltdtls(); return false;">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />">
		<ul>
			...
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/uss/olp/cns/CnsltDtlsRegistView.cm' />"  title="<spring:message code="button.create" /> <spring:message code="input.button" />"><spring:message code="button.create" /></a></span>
			</li>
		</ul>
	</div>
...
<input type="hidden" name ="nextUrl" value="<c:url value='/uss/olh/qna/QnaCnRegistView.cm' />">
...
<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>"/>
	...
	<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
	  <tr>
		...
		<td class="left">
		<form name="linkForm" method="post" action="<c:url value='/uss/olp/cns/CnsltInqireCoUpdt.cm'/>">
		...
		<%-- <a href="<c:url value='/uss/olp/cns/CnsltInqireCoUpdt.cm'/>?pageIndex=${searchVO.pageIndex}&cnsltId=${resultInfo.cnsltId}&passwordConfirmAt="><c:out value="${resultInfo.cnsltSj}"/></a> --%>
		</td>
