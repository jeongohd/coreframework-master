...
<script type="text/javascript">
	...
	function fn_egov_inqire_ntwrksvcmntrng(sysIp, sysPort) {
		...
		document.frm.action = "<c:url value='/utl/sys/nsm/selectNtwrkSvcMntrng.cm'/>";
		document.frm.submit();	
	}

	function fn_egov_insert_ntwrksvcmntrng(){	
		document.frm.action = "<c:url value='/utl/sys/nsm/addNtwrkSvcMntrng.cm'/>";
		document.frm.submit();
	}
...
<div class="board">
	...
	<form name="frm" method="post" action="<c:url value='/utl/sys/nsm/selectNtwrkSvcMntrngList.cm'/>">
	...
	<div class="search_box" title="<spring:message code='common.searchCondition.msg' />">
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/utl/sys/nsm/addNtwrkSvcMntrng.cm'/>" onclick="fn_egov_insert_ntwrksvcmntrng('1'); return false;" title='<spring:message code="button.create" />'><spring:message code="button.create" /></a></span><!-- 등록 -->
				<span class="btn_b"><a href="<c:url value='/utl/sys/nsm/selectNtwrkSvcMntrngLogList.cm'/>" onclick="fn_egov_log_ntwrksvcmntrng(); return false;" title='<spring:message code="button.log" />'><spring:message code="button.log" /></a></span><!-- 로그 -->
			</li>
		</ul>
	</div>
		...
		<tbody>
			<c:forEach var="result" items="${resultList}" varStatus="status">
			  <tr>
			    ...
			    <td>
			     <form name="ntwrkSvcMntrngVO" method="post" action="<c:url value='/utl/sys/nsm/selectNtwrkSvcMntrng.cm'/>">
			    	<input name="pageIndex" type="hidden" value="<c:out value='${searchVO.pageIndex}'/>">
