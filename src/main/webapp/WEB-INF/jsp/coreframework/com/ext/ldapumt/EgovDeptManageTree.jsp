...
<html>
	<head>
		...
		<link rel="stylesheet" href="<c:url value='/html/coreframework/com/ext/ldapumt/themes/default/style.css' />" />
		<style>
		...
		#tree .dept { background:url('<c:url value="/html/coreframework/com/ext/ldapumt/image/file_sprite.png" />') right bottom no-repeat; }
		#tree .user { background:url('<c:url value="/html/coreframework/com/ext/ldapumt/image/user.png" />') 0 0 no-repeat; }
		</style>
	</head>
	<body>
		...
		<script language="javascript" src="<c:url value='/html/coreframework/com/ext/ldapumt/libs/jquery.js' />"></script>
		<script language="javascript" src="<c:url value='/html/coreframework/com/ext/ldapumt/jstree.js' />"></script>
		...
		$(function () {
			...
			$('#tree')
				.jstree({
					'core' : {
						'data' : {
							'url' : '<c:url value="/ext/ldapumt/dpt/getDeptManageSublist.cm"/>',
							'data' : function (node) {
				...
				.on('delete_node.jstree', function (e, data) {
					$.get('<c:url value="/ext/ldapumt/dpt/deleteNode.cm" />', { 'dn' : data.node.id })
						.fail(function () {
				...
				.on('create_node.jstree', function (e, data) {
					...
					if(data.node.type == "default") {
						url="<c:url value='/ext/ldapumt/dpt/createNode.cm' />";
					} else {
						url="<c:url value='/ext/ldapumt/dpt/createUserNode.cm' />";
					}
				...
				.on('rename_node.jstree', function (e, data) {
					if(data.node.icon == "dept") {
						url="<c:url value='/ext/ldapumt/dpt/renameNode.cm' />";
					} else {
						url="<c:url value='/ext/ldapumt/dpt/renameUserNode.cm' />";
					}
				...
				.on('move_node.jstree', function (e, data) {
					$.get('<c:url value="/ext/ldapumt/dpt/moveOrgNode.cm" />', { 'id' : data.node.id, 'parent' : data.parent })
						.done(function (d) {
							data.instance.refresh();
						})
						.fail(function () {
				...
				.on('changed.jstree', function (e, data) {
					if(data && data.selected && data.selected.length) {
						...
						// 선택이 부서일 경우
						if(data.node.icon == "dept") {
							url = "<c:url value='/ext/ldapumt/dpt/coreDeptManage.cm' />";
							htmlfile = "<c:url value='/html/coreframework/com/ext/ldapumt/dept_html.jsp' />";
						}
							
						if(data.node.icon == "user") {
							url = "<c:url value='/ext/ldapumt/dpt/getUserManage.cm' />";
							htmlfile = "<c:url value='/html/coreframework/com/ext/ldapumt/user_html.jsp' />";
						}
			...
			function modifyOrgManage(str){
				var url="";
				if(str=='dept') {
					url = '<c:url value="/ext/ldapumt/dpt/modifyDeptManage.cm" />';
				} else {
					url = '<c:url value="/ext/ldapumt/dpt/modifyUserManage.cm" />';
				}
