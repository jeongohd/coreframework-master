...
<%--
  ...
  */
  /* Image Path 설정 */
  //String imagePath_icon   = "/images/coreframework/com/sym/prm/icon/";
  //String imagePath_button = "/images/coreframework/com/sym/prm/button/";
--%>
...
<c:set var="ImgUrl" value="/images/coreframework/com/sym/prm/"/>
<c:set var="CssUrl" value="/css/coreframework/com/sym/prm/"/>
<html lang="ko">
<head>
...
function searchFileNm() {
	...
	window.open("<c:url value='/sym/prm/coreProgrmListSearch.cm' />",'','width=500,height=600');
}
...
function insertProgrmChangeRequst(form) {
	if(confirm("<spring:message code="common.save.msg" />")){
		...
		if(!validateProgrmManageDtlVO(form)){
            ...
            form.action = "<c:url value='/sym/prm/EgovProgramChangRequstStre.cm'/>";
			form.submit();
		}
	}
 ...
 ******************************************************** */
function selectList(){
	location.href = "<c:url value='/sym/prm/EgovProgramChangeRequstSelect.cm' />";
}
...
<form:form modelAttribute="progrmManageDtlVO" name="progrmManageDtlVO" action ="<c:url value='/sym/prm/EgovProgramChangRequstStre.cm'/>" method="post">
...
<div class="wTableFrm">
	...
	<table class="wTable">
		...
		<tr>
			...
			<td class="left">
				...
				<a href="<c:url value='/sym/prm/coreProgrmListSearch.cm'/>?tmp_SearchElementName=progrmFileNm" target="_blank" onclick="searchFileNm(); return false;" style="selector-dummy:expression(this.hideFocus=false);"  title="새 창으로 이동"><!-- 새 창으로 이동 -->
				<img src="<c:url value='/images/coreframework/com/cmm/icon/search2.gif' />" alt='(<spring:message code="comSymPrm.programChangRequstStre.searchProgrmFileNm"/>)' width="15" height="15" /></a><!-- 프로그램파일명 검색 -->
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/sym/prm/EgovProgramChangeRequstSelect.cm'/>" onclick="selectList(); return false;"><spring:message code="button.list" /></a></span><!-- 목록 -->
	</div>
