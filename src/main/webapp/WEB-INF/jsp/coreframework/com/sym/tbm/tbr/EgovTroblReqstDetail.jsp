...
function fncSelectTroblReqstList() {
    ...
    varFrom.action = "<c:url value='/sym/tbm/tbr/selectTroblReqstList.cm'/>";
    varFrom.submit();       
}
...
function fncTroblReqstUpdateView(troblId) {
    ...
    varFrom.action = "<c:url value='/sym/tbm/tbr/updtViewTroblReqst.cm'/>";
    varFrom.submit();
}
...
function fncTroblReqstDelete(troblId) {
    ...
    varFrom.troblId.value = troblId;
    varFrom.action = "<c:url value='/sym/tbm/tbr/removeTroblReqst.cm'/>";
...
function fncTroblReqstRequst(troblId) {
    ...
    varFrom.troblId.value = troblId;
    varFrom.action = "<c:url value='/sym/tbm/tbr/requstTroblReqst.cm'/>";
...
function fncTroblReqstRequstCancl(troblId) {
    ...
    varFrom.troblId.value = troblId;
    varFrom.action = "<c:url value='/sym/tbm/tbr/requstTroblReqstCancl.cm'/>";
...
<div class="wTableFrm">
	...
	<form:form modelAttribute="troblReqst" method="post" action="${pageContext.request.contextPath}/sym/tbm/tbr/updtViewTroblReqst.cm">
	<!-- 등록폼 -->
	<table class="wTable">
		<colgroup>
			<col style="width:16%" />
	...
	<div class="btn">
		<c:if test="${troblReqst.processSttus == 'A'}">
			...
			<span class="btn_s"><a href="<c:url value='/sym/tbm/tbr/removeTroblReqst.cm'/>?troblId=<c:out value='${troblReqst.troblId}'/>" onclick="fncTroblReqstDelete('${troblReqst.troblId}'); return false;"><spring:message code="button.delete" /></a></span><!-- 삭제 -->
			<span class="btn_s"><a href="<c:url value='/sym/tbm/tbr/requstTroblReqst.cm'/>?troblId=<c:out value='${troblReqst.troblId}'/>" onclick="fncTroblReqstRequst('${troblReqst.troblId}'); return false;"><spring:message code="comSymTbmTbr.troblReqstDetail.request"/></a></span><!-- 요청 -->
		</c:if>
        <c:if test="${troblReqst.processSttus == 'R'}">
        	<span class="btn_s2"><a href="<c:url value='/sym/tbm/tbr/requstTroblReqstCancl.cm'/>?troblId=<c:out value='${troblReqst.troblId}'/>" onclick="fncTroblReqstRequstCancl('${troblReqst.troblId}'); return false;"><spring:message code="comSymTbmTbr.troblReqstDetail.cancelRequest"/></a></span><!-- 요청취소 -->
        </c:if>
        <span class="btn_s"><a href="<c:url value='/sym/tbm/tbr/selectTroblReqstList.cm'/>?pageIndex=<c:out value='${troblReqstVO.pageIndex}'/>&amp;strTroblNm=<c:out value="${troblManageVO.strTroblNm}"/>" onclick="fncSelectTroblReqstList(); return false;"><spring:message code="button.list" /></a></span><!-- 목록 -->
	</div>
