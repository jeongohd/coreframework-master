...
function fMenuCreatManageSelect(){
    document.menuCreatManageForm.action = "<c:url value='/sym/mnu/mcm/EgovMenuCreatManageSelect.cm'/>";
...
<div class="board">
	...
	<form name="menuCreatManageForm" action ="<c:url value='/sym/mnu/mcm/EgovMenuCreatManageSelect.cm'/>" method="post">
		...
		<tbody>
			<c:forEach var="result" items="${list_menumanage}" varStatus="status">
			  <tr>
			    ...
			    <td>
			       <a class="btn02" href="<c:url value='/sym/mnu/mcm/EgovMenuCreatSelect.cm'/>?authorCode='<c:out value="${result.authorCode}"/>'"  onclick="selectMenuCreat('<c:out value="${result.authorCode}"/>'); return false;">메뉴생성</a>
			    </td>
			  </tr>
			 </c:forEach>
		</tbody>
