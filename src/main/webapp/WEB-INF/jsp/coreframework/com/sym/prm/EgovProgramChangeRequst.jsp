...
//	document.progrmChangeRequstForm.searchKeyword.value =
	...
	document.progrmChangeRequstForm.action = "<c:url value='/sym/prm/EgovProgramChangeRequstSelect.cm'/>";
...
function insertChangeRequst() {
   	document.progrmChangeRequstForm.action = "<c:url value='/sym/prm/EgovProgramChangRequstStre.cm'/>";
   	document.progrmChangeRequstForm.submit();
}
...
function selectUpdtChangeRequstListDetail(progrmFileNm, tmpNo) {
	...
	document.progrmChangeRequstForm.action = "<c:url value='/sym/prm/EgovProgramChangRequstDetailSelect.cm'/>";
	document.progrmChangeRequstForm.submit();
}
...
<div class="board">
	...
	<form name="progrmChangeRequstForm" action ="<c:url value='/sym/prm/EgovProgramChangeRequstSelect.cm'/>?pageIndex=1" method="post">
	...
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/sym/prm/EgovProgramChangRequstStre.cm'/>" onclick="insertChangeRequst(); return false;" title='<spring:message code="button.create" />'><spring:message code="button.create" /></a></span><!-- 등록 -->
			</li>
		</ul>
	</div>
		...
		<tbody>
			...
			<c:forEach var="result" items="${list_changerequst}" varStatus="status">
			  <tr>
			    ...
			    <td>
			      <form name="item" method="post" action="<c:url value='/sym/prm/EgovProgramChangRequstDetailSelect.cm'/>">
