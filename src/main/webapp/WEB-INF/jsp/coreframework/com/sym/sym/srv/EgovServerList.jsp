...
function fncSelectServerList(pageNo){
    ...
    document.listForm.action = "<c:url value='/sym/sym/srv/selectServerList.cm'/>";
    document.listForm.submit();
}

function fncSelectServer(serverId) {
    document.listForm.serverId.value = serverId;
    document.listForm.action = "<c:url value='/sym/sym/srv/getServer.cm'/>";
    document.listForm.submit();   
}
...
function fncAddServerInsert() {
    ...
    document.listForm.action = "<c:url value='/sym/sym/srv/addViewServer.cm'/>";
    document.listForm.submit(); 
}
...
function windowOpen(serverId) {
	var url = "<c:url value='/sym/sym/srv/selectServerEqpmnRelateList.cm'/>?strServerId="+serverId;
...
<div class="board">
	...
	<form name="listForm" action="<c:url value='/sym/sym/srv/selectServerList.cm'/>" method="post">
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/sym/sym/srv/addViewServer.cm'/>?pageIndex=<c:out value='${serverVO.pageIndex}'/>&amp;strServerNm=<c:out value="${serverVO.strServerNm}"/>" onclick="fncAddServerInsert(); return false;" title='<spring:message code="button.create" />'><spring:message code="button.create" /></a></span><!-- 등록 -->
			</li>
		</ul>
	</div>
	...
	<table class="board_list">
		...
		<tbody>
			<c:forEach var="server" items="${serverList}" varStatus="status">
			  <tr>
			    <td>
			        <form name="item" method="post" action="<c:url value='/sym/sym/srv/getServer.cm'/>">
			    ...
			    <td><a href="#" onclick="windowOpen('<c:out value="${server.serverId}"/>')"><img src="<c:url value='/images/coreframework/com/cmm/icon/search2.gif'/>" alt="" /></a></td>
			  </tr>
			 </c:forEach>
		</tbody>
	</table>
