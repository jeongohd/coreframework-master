...
function fncSelectServerEqpmnList() {
    ...
    varFrom.action = "<c:url value='/sym/sym/srv/selectServerEqpmnList.cm'/>";
    varFrom.submit();       
}
...
function fncServerEqpmnUpdateView(serverEqpmnId) {
    ...
    varFrom.action = "<c:url value='/sym/sym/srv/updtViewServerEqpmn.cm'/>";
    varFrom.submit();
}
...
function fncServerEqpmnDelete(serverEqpmnId) {
    var varFrom = document.getElementById("serverEqpmn");
    varFrom.serverEqpmnId.value = serverEqpmnId;
    varFrom.action = "<c:url value='/sym/sym/srv/removeServerEqpmn.cm'/>";
...
<form:form modelAttribute="serverEqpmn" method="post" action="${pageContext.request.contextPath}/sym/sym/srv/updtViewServerEqpmn.cm">
...
<div class="wTableFrm">
	...
	<!-- 하단 버튼 -->
	<div class="btn">
		<span class="btn_s"><a href="<c:url value='/sym/sym/srv/updtViewServerEqpmn.cm'/>?serverEqpmnId=<c:out value='${serverEqpmn.serverEqpmnId}'/>" onclick="fncServerEqpmnUpdateView('${serverEqpmn.serverEqpmnId}'); return false;"><spring:message code="button.update" /></a></span><!-- 수정 -->
		<span class="btn_s"><a href="<c:url value='/sym/sym/srv/removeServerEqpmn.cm'/>?serverEqpmnId=<c:out value='${serverEqpmn.serverEqpmnId}'/>" onclick="fncServerEqpmnDelete('${serverEqpmn.serverEqpmnId}'); return false;"><spring:message code="button.delete" /></a></span><!-- 삭제 -->
		<span class="btn_s"><a href="<c:url value='/sym/sym/srv/selectServerEqpmnList.cm'/>?pageIndex=<c:out value='${serverEqpmnVO.pageIndex}'/>&amp;strServerEqpmnNm=<c:out value="${serverEqpmnVO.strServerEqpmnNm}"/>" onclick="fncSelectServerEqpmnList(); return false;"><spring:message code="button.list" /></a></span><!-- 목록 -->
	</div>
