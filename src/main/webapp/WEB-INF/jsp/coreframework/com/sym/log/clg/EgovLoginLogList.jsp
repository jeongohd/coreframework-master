...
function fn_egov_select_linkPage(pageNo){
	...
	document.LoginLogForm.action = "<c:url value='/sym/log/clg/SelectLoginLogList.cm'/>";
...
function fn_egov_search_loginLog(){
	...
	vFrom.action = "<c:url value='/sym/log/clg/SelectLoginLogList.cm'/>";
	vFrom.submit();
}
/* ********************************************************
 * 상세회면 처리 함수
...
function fn_egov_detail_loginLog(logId) {
  	...
  	var top = 0;
  	var left = 0;
  	var url = "<c:url value='/sym/log/clg/SelectLoginLogDetail.cm' />?logId="+logId;
...
<form name="LoginLogForm" action="<c:url value='/sym/log/clg/SelectLoginLogList.cm'/>" method="post" onSubmit="fn_egov_search_loginLog(); return false;">
<div class="board">
	...
	<tr>
		...
		<td>
		<img src="<c:url value='/images/coreframework/com/cmm/btn/btn_search.gif'/>"  class="cursor" onclick="fn_egov_detail_loginLog('<c:out value="${result.logId}"/>'); return false;" alt="<spring:message code="title.detail" />"  title="<spring:message code="title.detail" />">
		</td>
	</tr>
	</c:forEach>
	</tbody>
