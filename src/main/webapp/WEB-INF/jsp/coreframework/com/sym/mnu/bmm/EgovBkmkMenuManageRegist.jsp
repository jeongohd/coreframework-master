...
<script type="text/javascript">
	function fn_egov_regist_bkmkInf(){
		...
		if(confirm("<spring:message code='comSymMnuBmm.bkmkMenuManageRegist.validate.confirm.regist' />")){ //등록하시겠습니까?
			document.bkmk.action = "<c:url value='/sym/mnu/bmm/registBkmkInf.cm'/>";
	...
	function fn_egov_select_bkmkInfs(){
		document.bkmk.action = "<c:url value='/sym/mnu/bmm/selectBkmkMenuManageList.cm'/>";
		document.bkmk.submit();
	}
	...
	function fn_egov_inqire_menu(){
		...
		var width = 780;
		var height = 700;
		var url = "<c:url value='/sym/mnu/bmm/openPopup.cm?requestUrl=/sym/mnu/bmm/selectMenuList.cm'/>&width="+(width-20)+"&height="+(height-20);
		...
		if(retVal != null){
			...
			document.bkmk.action = "<c:url value='/sym/mnu/bmm/addBkmkInf.cm'/>";
			document.bkmk.submit();
		}
	}
...
<script type="text/javascript">
    $(document).ready(function () {
        $('#popupInqireMenu').click(function (e) {
            ...
            var page = "<c:url value='/sym/mnu/bmm/selectMenuList.cm'/>";
...
<form  name="bkmk" method="post" action = "<c:url value='/sym/mnu/bmm/registBkmkInf.cm'/>">
...
<div class="wTableFrm">
		...
		<tr>
			...
			<td class="left">
			    <input name="menuNm" type="text" size="30"  value="<c:out value="${bkmkMenuManage.menuNm}" escapeXml="false" />" maxlength="60" style="width: 237px" readonly="readonly" class="readOnlyClass" title="메뉴명입력"/>
	      		<!-- <a href="javascript:fn_egov_inqire_menu();" style="selector-dummy: expression(this.hideFocus=false);"><img src="<c:url value='/images/coreframework/com/cmm/icon/search2.gif' />"
	     		...
	     		<a id="popupInqireMenu" title="메뉴선택" style="selector-dummy: expression(this.hideFocus=false);"><img src="<c:url value='/images/coreframework/com/cmm/icon/search2.gif' />"
	...
	<div class="btn">
		...
		<span class="btn_s"><a href="<c:url value='/sym/mnu/bmm/selectBkmkMenuManageList.cm'/>?pageIndex=<c:out value='${searchVO.pageIndex}'/>" onclick="fn_egov_select_bkmkInfs(); return false;"><spring:message code="button.list" /></a></span><!-- 목록 -->
	</div>
