...
function fn_egov_select_linkPage(pageNo){
	...
	document.SysLogForm.action = "<c:url value='/sym/log/lgm/coreSysLogList.cm'/>";
...
function fn_egov_search_sysLog(){
	...
	vFrom.action = "<c:url value='/sym/log/lgm/coreSysLogList.cm'/>";
	vFrom.submit();
}
/* ********************************************************
 * 상세회면 처리 함수
...
function fn_egov_inquire_sysLog(requstId) {
  	...
  	var top = 0;
  	var left = 0;
  	var url = "<c:url value='/sym/log/lgm/coreSysLog.cm' />?requstId="+requstId;
...
<form name="SysLogForm" action="<c:url value='/sym/log/lgm/coreSysLogList.cm'/>" method="post" onSubmit="fn_egov_search_sysLog(); return false;">
<div class="board">
	...
	<tr>
		...
		<td>
		<img src="<c:url value='/images/coreframework/com/cmm/btn/btn_search.gif'/>" class="cursor" onclick="fn_egov_inquire_sysLog('<c:out value="${result.requstId}"/>'); return false;" alt="<spring:message code="title.detail" />"  title="<spring:message code="title.detail" />">
		</td>
	</tr>
	</c:forEach>
	</tbody>
