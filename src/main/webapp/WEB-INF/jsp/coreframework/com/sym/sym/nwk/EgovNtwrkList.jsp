...
function fncSelectNtwrkList(pageNo){
    ...
    document.listForm.action = "<c:url value='/sym/sym/nwk/selectNtwrkList.cm'/>";
    document.listForm.submit();
}

function fncSelectNtwrk(ntwrkId) {
    document.listForm.ntwrkId.value = ntwrkId;
    document.listForm.action = "<c:url value='/sym/sym/nwk/getNtwrk.cm'/>";
    document.listForm.submit();   
}
...
function fncAddNtwrkInsert() {
    ...
    document.listForm.action = "<c:url value='/sym/sym/nwk/addViewNtwrk.cm'/>";
    document.listForm.submit(); 
}
...
<form name="listForm" action="<c:url value='/sym/sym/nwk/selectNtwrkList.cm'/>" method="post">
	<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
		<ul>
			<li>
				...
				<span class="btn_b"><a href="<c:url value='/sym/sym/nwk/addViewNtwrk.cm'/>?pageIndex=<c:out value='${ntwrkVO.pageIndex}'/>&amp;searchKeyword=<c:out value="${ntwrkVO.searchKeyword}"/>&amp;searchCondition=1" onclick="fncAddNtwrkInsert(); return false;" title='<spring:message code="button.create" />'><spring:message code="button.create" /></a></span><!-- 등록 -->
			</li>
		</ul>
	</div>
	...
	<table class="board_list">
		...
		<tbody>
			<c:forEach var="ntwrk" items="${ntwrkList}" varStatus="status">
			  <tr>
			    <td>
			        <form name="item" method="post" action="<c:url value='/sym/sym/nwk/getNtwrk.cm'/>">
