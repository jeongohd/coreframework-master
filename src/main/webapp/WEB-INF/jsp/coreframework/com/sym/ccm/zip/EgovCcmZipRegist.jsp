...
function fn_egov_list_Zip(){
	location.href = "<c:url value='/sym/ccm/zip/EgovCcmZipList.cm' />";
}
/* ********************************************************
 * 등록처리
 ******************************************************** */
...
function goAddSearch() {
	// 호출된 페이지(jusopopup.jsp)에서 실제 주소검색URL(http://www.juso.go.kr/addrlink/addrLinkUrl.cm)를 호출하게 됩니다.
    var pop = window.open("<c:url value='/sym/ccm/zip/EgovAdressPop.cm' />","pop","width=570,height=420, scrollbars=yes, resizable=yes");
    
	// 모바일 웹인 경우, 호출된 페이지(jusopopup.jsp)에서 실제 주소검색URL(http://www.juso.go.kr/addrlink/addrMobileLinkUrl.cm)를 호출하게 됩니다.
...
<!-- 상단 타이틀  영역 -->
  ...
  <c:if test="${searchList == '1'}">
   <img src="<c:url value='/images/coreframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" style="vertical-align: middle" alt="제목아이콘이미지">&nbsp;${titleZip }</h1></td>
  </c:if>
  <c:if test="${searchList == '2'}">
   <img src="<c:url value='/images/coreframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" style="vertical-align: middle" alt="제목아이콘이미지">&nbsp;${titleRdmn }</h1></td>
  </c:if>
