...
<html lang="ko">
	<head>
		...
		function linkPage(pageNo){
			...
			form.action = "<c:url value='/dam/map/tea/EgovComDamMapTeamList.cm'/>";
		   	form.submit();
		}
		/* ********************************************************
		 * 조회 처리 
		 ...
		 ******************************************************** */
		function fnRegist(){
			location.href = "<c:url value='/dam/map/tea/EgovComDamMapTeamRegist.cm'/>";
		}
		-->
		</script>
	</head>
	...
	<div class="board">
		...
		<form name="listForm" action="<c:url value='/dam/map/tea/EgovComDamMapTeamList.cm'/>" method="post">
		<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
			<ul>
				<li>
					...
					<span class="btn_b"><a href="<c:url value='/dam/map/tea/EgovComDamMapTeamRegist.cm'/>" onclick="" title='<spring:message code="button.create" />'><spring:message code="button.create" /></a></span><!-- 등록 -->
				</li>
			</ul>
		</div>
			...
			<tbody>
				<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
					<tr>				
						...
						<td>
						<a href="<c:url value='/dam/map/tea/EgovComDamMapTeamDetail.cm'/>?pageIndex=${searchVO.pageIndex}&amp;orgnztId=${resultInfo.orgnztId}"><c:out value="${resultInfo.orgnztNm}"/></a>
						</td>
