...
<html lang="ko">
	<head>
		...
		function linkPage(pageNo){
			...
			document.listForm.action = "<c:url value='/dam/per/EgovComDamPersonalList.cm'/>";
		 ...
		 ******************************************************** */
		function fnRegist(){
			location.href = "<c:url value='/dam/per/EgovComDamPersonalRegist.cm' />";
		}
		-->
		</script>
	</head>
	...
	<!-- 자바스크립트 경고 태그  -->
		...
		<div class="board">
			...
			<form name="listForm" action="<c:url value='/dam/per/EgovComDamPersonalList.cm'/>" method="post">
			...
			<div class="search_box" title="<spring:message code="common.searchCondition.msg" />"><!-- 이 레이아웃은 하단 정보를 대한 검색 정보로 구성되어 있습니다. -->
				<ul>
					<li>
						...
						<span class="btn_b"><a href="<c:url value='/dam/per/EgovComDamPersonalRegistView.cm'/>" onclick="" title='<spring:message code="button.create" />'><spring:message code="button.create" /></a></span><!-- 등록 -->
					</li>
				</ul>
			</div>
				...
				<tbody>
					<c:forEach items="${resultList}" var="resultInfo" varStatus="status">
						<tr>
							...
							<td>
								<a href="<c:url value='/dam/per/EgovComDamPersonal.cm'/>?pageIndex=${searchVO.pageIndex}&amp;knoId=${resultInfo.knoId}"><c:out value="${resultInfo.knoNm}"/></a>
							</td>
